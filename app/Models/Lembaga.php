...
class Lembaga extends Model
{
    ...
    protected static function boot()
    {
        ...
        static::creating(function ($model) {
            ...
            if (empty($model->kode_lembaga)) {
                $model->kode_lembaga = self::generateKodeLembaga();
            }
    ...
    public static function generateKodeLembaga()
    {
        $prefix = 'LMBG';
        $year = date('Y');

        $lastLembaga = self::whereYear('created_at', $year)
            ->orderBy('id', 'desc')
            ->first();
    ...
    public function getFotoUrlsAttribute()
    {
        ...
        if (empty($urls)) {
            return [asset('images/default-lembaga.jpg')];
        }
     ...
     */
    public function getUsiaLembagaAttribute()
    {
        if ($this->tahun_berdiri) {
            $currentYear = date('Y');
    ...
    public function scopeSearch($query, $term)
    {
        return $query->where(function ($q) use ($term) {
            $q->where('nama', 'like', "%{$term}%")
                ->orWhere('kode_lembaga', 'like', "%{$term}%")
    ...
    {
        return $this->hasMany(\App\Models\Amil::class, 'lembaga_id');
    }

    // Relasi ke Mustahik
    public function mustahiks()
    {
        return $this->hasMany(\App\Models\Mustahik::class, 'lembaga_id');
    }

    // Relasi ke Transaksi Penerimaan
    public function transaksiPenerimaan()
    {
        return $this->hasMany(\App\Models\TransaksiPenerimaan::class, 'lembaga_id');
    }
