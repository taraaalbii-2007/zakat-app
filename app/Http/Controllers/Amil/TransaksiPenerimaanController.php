...
class TransaksiPenerimaanController extends Controller
{
    ...
    public function createDijemput(Request $request)
    {
        ...
        // Dapatkan daftar amil (untuk admin lembaga)
        $amilList = Amil::byLembaga($this->lembaga->id)
            ->with('pengguna')
            ->where('status', 'aktif')
            ->get();
        ...
        $isAmilUser = $this->user->isAmil() && $this->amil;
        $currentAmil = null;

        if ($isAmilUser) {
            $currentAmil = $this->amil;
        }
        ...
        // Gunakan view khusus untuk create-dijemput
        return view('amil.transaksi-penerimaan.create-dijemput', compact(
            'mode',
            'jenisZakatList',
            'programZakatList',
            'amilList',
            ...
            'qrisConfig',
            'breadcrumbs',
            'currentAmil',      // tambahkan ini
            'isAmilUser'        // tambahkan ini
        ));
    }
    ...
    // ================================================================
    // STORE DIJEMPUT - PERBAIKAN (TANPA VALIDASI JENIS ZAKAT)
    // ================================================================
    public function storeDijemput(Request $request)
    {
        ...
        try {
            // HANYA validasi data dasar untuk request penjemputan
            // TIDAK ADA validasi jenis_zakat_id, tipe_zakat_id, metode_pembayaran
            $rules = [
            ...
            // SET JUMLAH = 0 (akan diisi amil nanti)
            $transaksi->jumlah             = 0;
            $transaksi->jumlah_dibayar     = 0;
            $transaksi->jumlah_infaq       = 0;
            $transaksi->has_infaq          = false;
            ...
            // STATUS PENDING - menunggu penjemputan
            $transaksi->status             = 'pending';
            $transaksi->konfirmasi_status  = null;

            // JANGAN SET field zakat (biarkan NULL)
            // $transaksi->jenis_zakat_id   = null; // default sudah null
            // $transaksi->tipe_zakat_id    = null; // default sudah null
            // $transaksi->metode_pembayaran = null; // default sudah null
            ...
            Log::info('Request penjemputan berhasil disimpan', [
                ...
                'status_penjemputan' => $transaksi->status_penjemputan,
            ]);
    ...
    public function completePickup(Request $request, $uuid)
    {
        ...
        try {
            ...
            if ($transaksi->status === 'verified' && $transaksi->muzakki_email) {
                $this->kirimEmailKwitansi($transaksi);
            }
    ...
    public function update(Request $request, $uuid)
    {
        ...
        try {
            ...
            if ($isLengkapiZakat) {
                ...
                if ($isFidyah) {
                    ...
                    if ($request->fidyah_tipe == 'tunai') {
                        ...
                        // ✅ PERBAIKAN: Semua metode pembayaran di penjemputan langsung verified
                        $transaksi->status = 'verified';
                        $transaksi->verified_by = $this->user->id;
                        $transaksi->verified_at = now();
                        ...
                        // Untuk transfer/qris, langsung dikonfirmasi
            ...
            // ✅ KIRIM EMAIL KWITANSI jika status verified
            if ($isLengkapiZakat && $transaksi->status === 'verified' && $transaksi->muzakki_email) {
                $this->kirimEmailKwitansi($transaksi);
            }

            $message = $isLengkapiZakat
                ? ($isFidyah
                    ? 'Data fidyah berhasil disimpan. Transaksi selesai.'
                    : ($isBeras
                        ? 'Data zakat beras berhasil disimpan. Transaksi selesai.'
                        : 'Data zakat berhasil dilengkapi. Transaksi selesai.'))
        ...
        try {
            ...
            if ($transaksi->muzakki_email) {
                try {
                    ...
                    Log::warning('Gagal kirim email kwitansi konfirmasi: ' . $e->getMessage(), [
                        'no_transaksi' => $transaksi->no_transaksi,
                    ]);
                }
            }
    ...
    // ================================================================
    // EXPORT EXCEL — di dalam controller (misal: TransaksiPenerimaanController)
    // ================================================================
    ...
    public function exportExcel(Request $request)
    {
        try {
            $filters = $request->only([
                ...
                'end_date',
                'metode_penerimaan',
                'fidyah_tipe',          // ← sudah ada, pastikan diteruskan
            ]);

            // Pastikan relasi amil ter-load agar nama amil tampil di info ekspor
            $user = $this->user->loadMissing('amil');

            $filename = 'transaksi-penerimaan-' . date('Y-m-d-His') . '.xlsx';

            return Excel::download(
                new TransaksiPenerimaanExport($filters, $user, $this->lembaga),
                $filename
            );
        } catch (\Exception $e) {
    ...
    protected function isiMetodePembayaranPickup(TransaksiPenerimaan $transaksi, Request $request, bool $isPembayaranBeras): void
    {
        if ($isPembayaranBeras) {
            ...
            // ✅ PERBAIKAN: Untuk transfer/qris di pickup, langsung dikonfirmasi
            if (in_array($metodePembayaran, ['transfer', 'qris'])) {
                $transaksi->konfirmasi_status = 'dikonfirmasi';
            }
        }
    ...
    /**
     * Kirim email kwitansi ke muzakki
     */
    protected function kirimEmailKwitansi(TransaksiPenerimaan $transaksi): void
    {
        try {
            $signedUrl = \Illuminate\Support\Facades\URL::temporarySignedRoute(
                'kwitansi.download',
            ...
            Log::info('Email kwitansi terkirim', [
                'no_transaksi' => $transaksi->no_transaksi,
                'email' => $transaksi->muzakki_email,
                'status' => $transaksi->status
            ]);
        } catch (\Exception $e) {
            Log::error('Gagal kirim email kwitansi: ' . $e->getMessage(), [
                'no_transaksi' => $transaksi->no_transaksi,
                'error' => $e->getMessage()
            ]);
        }
