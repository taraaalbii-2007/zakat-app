...
@push('styles')
    <style>
        ...
        .or-row span {
            ...
            color: #9ca3af;
            /* ← sedikit lebih gelap agar terbaca */
            white-space: nowrap;
            letter-spacing: .02em;
        }
    ...
    <form method="POST" action="{{ route('login') }}" id="login-form" novalidate>
        ...
        <div class="lg-group">
            ...
            <div class="lg-wrap {{ $errors->has('login') ? 'err' : '' }}">
                ...
                <input type="text" id="login" name="login" value="{{ old('login') }}" required autocomplete="off"
                    autofocus class="lg-input {{ $errors->has('login') ? 'err' : '' }}"
