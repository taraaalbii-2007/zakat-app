...
<style>
    ...
    .lg-input {
        ...
        font-size: .82rem; font-weight: 400; color: #111827;
        outline: none;
    ...
    .lg-input:disabled {
        ...
        cursor: not-allowed; border-color: #f3f4f6;
    }
    ...
    select.lg-input {
        ...
        padding-right: 2.25rem; padding-left: .875rem;
        cursor: pointer;
    }
    select.lg-input:focus { border-color: #16a34a; box-shadow: 0 0 0 3px rgba(22,163,74,.1); }
    select.lg-input:disabled { background-color: #f9fafb; color: #9ca3af; cursor: not-allowed; }
    ...
    /* FOTO PROFIL PREVIEW (single, circular) */
    ...
    .cp-gallery-slot {
        aspect-ratio: 1; border: 1.5px dashed #e5e7eb;
    ...
    .cp-alert {
        ...
        font-size: .75rem; margin-bottom: 1.1rem;
    ...
    .btn-masuk {
        ...
        -webkit-appearance: none; margin-top: 1.25rem;
    }
    ...
    .cp-overlay-box {
        ...
        box-shadow: 0 8px 30px rgba(0,0,0,.12); min-width: 180px;
    }
    .cp-overlay-spinner {
...
<form method="POST"
        ...
        <div class="lg-group">
            ...
            <div class="lg-wrap">
                ...
                <input type="email" name="admin_email" id="admin_email"
                       ...
                       value="{{ old('admin_email', $pengguna->email) }}"
            ...
            @enderror
            <span class="lg-hint" id="adminEmailHelp">Default: {{ $maskedEmail }}</span>
        </div>
    ...
    <div class="lg-group">
        ...
        <div class="lg-wrap">
            ...
            <input type="text" name="nama_masjid" id="nama_masjid"
                   class="lg-input {{ $errors->has('nama_masjid') ? 'err' : '' }}"
    ...
    <div class="lg-group">
        ...
        <div class="lg-wrap">
            ...
            <input type="email" name="email_masjid" id="email_masjid"
                   class="lg-input {{ $errors->has('email_masjid') ? 'err' : '' }}"
    ...
    <div class="lg-group">
        ...
        <div class="lg-wrap textarea-wrap">
            ...
            <textarea name="sejarah" id="sejarah"
                      ...
                      rows="4" required
                      placeholder="Ceritakan sejarah singkat masjid...">{{ old('sejarah') }}</textarea>
        </div>
        @error('sejarah')
            <div class="lg-err">
                <svg fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
        ...
        <div class="lg-group">
            ...
            <div class="lg-wrap">
                ...
                <input type="number" name="kapasitas_jamaah" id="kapasitas_jamaah"
                       ...
                       value="{{ old('kapasitas_jamaah') }}" required min="1"
                       placeholder="500">
            </div>
    ...
    <div class="lg-group">
        ...
        <label class="cp-file-label" for="foto_masjid">
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                <path stroke-linecap="round" stroke-linejoin="round"
        ...
        @enderror
        <span class="lg-hint" style="margin-bottom:.5rem;">Maks. 5 foto · JPG, JPEG, PNG · Maks. 2MB per foto</span>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (usernameInput && !isGoogleUser) {
        ...
        usernameInput.addEventListener('input', function () {
            ...
            if (val.length < 6) {
                ...
                help.textContent = 'Minimal 6 karakter'; return;
            }
            if (!/^[a-zA-Z0-9_]+$/.test(val)) {
                help.className = 'lg-hint error';
                help.textContent = 'Hanya huruf, angka, dan underscore'; return;
            }
    ...
    if (adminFotoInput) {
        adminFotoInput.addEventListener('change', function (e) {
            ...
            if (!['image/jpeg','image/jpg','image/png'].includes(file.type)) {
                alert('Format harus JPG/JPEG/PNG'); this.value = ''; return;
            }
            if (file.size > 2 * 1024 * 1024) {
                alert('Ukuran maksimal 2MB'); this.value = ''; return;
            }
    ...
    if (fotoMasjidInput) {
        fotoMasjidInput.addEventListener('change', function (e) {
            ...
            if (files.length > 5) { alert('Maksimal 5 foto'); this.value = ''; return; }
            for (const f of files) {
                if (!['image/jpeg','image/jpg','image/png'].includes(f.type)) {
                    alert('Format harus JPG/JPEG/PNG'); this.value = ''; return;
                }
                if (f.size > 2 * 1024 * 1024) {
                    alert('Maks. 2MB per foto'); this.value = ''; return;
                }
            }

            masjidFileName.textContent = files.length > 1 ? `${files.length} foto dipilih` : files[0].name;
            ...
            /* Isi slot dengan preview */
            files.forEach((file, i) => {
                const reader = new FileReader();
                reader.onload = ev => {
                    const slot = document.getElementById(`slot${i + 1}`);
                    slot.innerHTML = `<img src="${ev.target.result}" alt="Foto Masjid ${i + 1}">`;
    ...
    provinsiSelect.addEventListener('change', async function () {
        ...
        setLoading(kotaSelect, 'Memuat…');
        try {
            const r = await (await fetch(`/api/wilayah/cities/${this.value}`)).json();
            if (r.success && r.data) { fillSelect(kotaSelect, r.data, 'Pilih Kota / Kabupaten'); kotaSelect.disabled = false; }
            else resetSelect(kotaSelect, 'Pilih Kota / Kabupaten');
        } catch { resetSelect(kotaSelect, 'Gagal memuat'); }
    });
    ...
    kotaSelect.addEventListener('change', async function () {
        ...
        kodePosInput.value = '';
        if (!this.value) return;
        setLoading(kecamatanSelect, 'Memuat…');
        try {
            const r = await (await fetch(`/api/wilayah/districts/${this.value}`)).json();
            if (r.success && r.data) { fillSelect(kecamatanSelect, r.data, 'Pilih Kecamatan'); kecamatanSelect.disabled = false; }
            else resetSelect(kecamatanSelect, 'Pilih Kecamatan');
        } catch { resetSelect(kecamatanSelect, 'Gagal memuat'); }
    });
    ...
    kecamatanSelect.addEventListener('change', async function () {
        ...
        kodePosInput.value = '';
        if (!this.value) return;
        setLoading(kelurahanSelect, 'Memuat…');
        try {
            const r = await (await fetch(`/api/wilayah/villages/${this.value}`)).json();
            if (r.success && r.data) { fillSelect(kelurahanSelect, r.data, 'Pilih Kelurahan / Desa'); kelurahanSelect.disabled = false; }
            else resetSelect(kelurahanSelect, 'Pilih Kelurahan / Desa');
        } catch { resetSelect(kelurahanSelect, 'Gagal memuat'); }
    });
    ...
    kelurahanSelect.addEventListener('change', async function () {
        ...
        if (!this.value) return;
        try {
            const r = await (await fetch(`/api/wilayah/postal-code/${this.value}`)).json();
            if (r.success && r.data?.postal_code) kodePosInput.value = r.data.postal_code;
        } catch {}
    });
    ...
    form.addEventListener('submit', async function (e) {
        ...
        try {
            @if($recaptchaSiteKey)
            const token = await grecaptcha.execute('{{ $recaptchaSiteKey }}', { action: 'complete_profile' });
