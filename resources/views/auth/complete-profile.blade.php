            ...
            <div class="lg-group">
                ...
                <div class="lg-wrap">
                    ...
                    <input type="tel" name="telepon" id="telepon"
                        ...
                        required maxlength="20" placeholder="021xxxxxxxx" inputmode="numeric" pattern="[0-9]+"
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            /* ── TELEPON — only digits ── */
            ['admin_telepon', 'telepon'].forEach(function(id) {
                ...
                el.addEventListener('keypress', function(e) {
                    if (!/[0-9]/.test(e.key) && !['Backspace', 'Delete', 'ArrowLeft', 'ArrowRight',
                            'Tab'
                        ].includes(e.key)) {
                        e.preventDefault();
                    }
                ...
                adminFotoInput.addEventListener('change', async function(e) {
                    ...
                    reader.onload = ev => {
                        ...
                        adminBadge.textContent =
                            `Dikompresi: ${(file.size/1024).toFixed(0)}KB ke ${(compressed.size/1024).toFixed(0)}KB`;
            ...
            function renderGallery() {
                for (let i = 1; i <= MAX_FOTO; i++) {
                    ...
                    if (file) {
                        ...
                        reader.onload = ev => {
                            slot.innerHTML =
                                `<img src="${ev.target.result}" alt="Foto ${i}"><button type="button" class="slot-remove" onclick="removeFoto(${i-1})" title="Hapus foto">×</button>`;
                ...
                lembagaFileName.textContent = count > 0 ? `${count} foto dipilih — klik untuk menambah` :
            ...
            if (fotoTrigger) {
                fotoTrigger.addEventListener('change', async function(e) {
                    ...
                    if (available <= 0) {
                        alert(`Maksimal ${MAX_FOTO} foto. Hapus foto yang ada.`);
                        this.value = '';
                        return;
                    }
                    const filesToProcess = newFiles.slice(0, available);
                    if (newFiles.length > available) alert(
