...
select.lg-input:not(:disabled) {
    ...
    color: #111827;
        ...
        textarea.lg-input {
            height: auto;
            padding: .65rem .875rem .65rem 2.6rem;
            resize: vertical;
            min-height: 78px;
            line-height: 1.5;
        }
        ...
        .cp-gallery-slot .sph svg {
            width: 18px;
            height: 18px;
            margin-bottom: .15rem;
        }

        .cp-gallery-slot .sph p {
            font-size: .62rem;
            margin: 0;
            color: #d1d5db;
        }

        /* Remove button on gallery slot */
        ...
        .cp-gallery-slot .slot-remove:hover {
            background: #ef4444;
        }

        /* Compress status badge */
        .compress-badge {
            font-size: .68rem;
            color: #16a34a;
            font-weight: 600;
...
@section('auth-content')
    ...
    <form method="POST" action="{{ route('complete-profile.store', $token) }}" enctype="multipart/form-data"
        id="completeProfileForm" novalidate>
        @csrf
            ...
            <div class="lg-group">
                ...
                <div class="lg-wrap">
                    ...
                    <input type="text" name="admin_nama" id="admin_nama"
                        class="lg-input {{ $errors->has('admin_nama') ? 'err' : '' }}" value="{{ old('admin_nama') }}"
                        required maxlength="255" placeholder="Ahmad Hidayat">
                </div>
                @error('admin_nama')
                    <div class="lg-err">
                        <svg fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="lg-group">
                ...
                <div class="lg-wrap">
                    ...
                    <input type="text" name="kode_pos" id="kode_pos"
                        class="lg-input {{ $errors->has('kode_pos') ? 'err' : '' }}" value="{{ old('kode_pos') }}"
                        required maxlength="5" pattern="[0-9]{5}" placeholder="00000">
                </div>
                @error('kode_pos')
                    <div class="lg-err">
                        <svg fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="lg-group">
                ...
                <div class="lg-wrap">
                    ...
                    <input type="tel" name="telepon" id="telepon"
                        ...
                        required maxlength="20" placeholder="(021) xxxxxxxx">
                </div>
                @error('telepon')
                    <div class="lg-err">
                        <svg fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="lg-group">
                ...
                <div class="lg-wrap">
                    ...
                    <input type="text" name="pendiri" id="pendiri"
                        class="lg-input {{ $errors->has('pendiri') ? 'err' : '' }}" value="{{ old('pendiri') }}"
                        required maxlength="255" placeholder="H. Maulana">
                </div>
                @error('pendiri')
                    <div class="lg-err">
                        <svg fill="currentColor" viewBox="0 0 20 20">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (adminFotoInput) {
                adminFotoInput.addEventListener('change', async function(e) {
                    ...
                    if (!file.type.startsWith('image/')) {
                        alert('Format harus berupa gambar');
                        this.value = '';
                        return;
                    }
                    ...
                    reader.onload = ev => {
                        adminFotoThumb.innerHTML =
                            `<img src="${ev.target.result}" alt="Foto Admin">`;
                        ...
                        adminBadge.textContent = `Dikompresi: ${origKB}KB ke ${compKB}KB`;
            ...
            function renderGallery() {
                for (let i = 1; i <= MAX_FOTO; i++) {
                    ...
                    if (file) {
                        ...
                        reader.onload = ev => {
                            slot.innerHTML =
                                `<img src="${ev.target.result}" alt="Foto ${i}">
                        <button type="button" class="slot-remove" onclick="removeFoto(${i - 1})" title="Hapus foto">x</button>`;
            ...
            if (fotoTrigger) {
                fotoTrigger.addEventListener('change', async function(e) {
                    ...
                    if (available <= 0) {
                        alert(`Maksimal ${MAX_FOTO} foto. Hapus foto yang ada terlebih dahulu.`);
                        this.value = '';
                        return;
                    }
                    ...
                    for (const f of filesToProcess) {
                        ...
                        if (f.size > 2 * 1024 * 1024) {
                            alert(`File "${f.name}" melebihi 2MB`);
                            continue;
                        }
                    }

                    const valid = filesToProcess.filter(f => f.type.startsWith('image/') && f.size <=
                        2 * 1024 * 1024);
                    if (!valid.length) {
                        this.value = '';
                        return;
            ...
            /* ══════════════════════════════════
               INJECT FILES KE HIDDEN INPUTS SEBELUM SUBMIT
            ══════════════════════════════════ */
            ...
            provinsiSelect.addEventListener('change', async function() {
                ...
                try {
                    ...
                    resetSelect(kotaSelect, 'Gagal memuat');
                }
            });
            ...
            kecamatanSelect.addEventListener('change', async function() {
                ...
                try {
                    ...
                    resetSelect(kelurahanSelect, 'Gagal memuat');
                }
            });
