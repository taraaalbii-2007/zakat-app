...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-xl overflow-hidden transition-all duration-300">
            ...
            <div class="px-5 py-4 border-b border-gray-100">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">Kelola dan konfigurasi transaksi penyaluran dari seluruh
                            lembaga</p>
                    </div>
            ...
            <!-- Filter Panel -->
            <div id="filterPanel"
                class="{{ request()->hasAny(['q', 'status', 'lembaga_id', 'metode_penyaluran', 'start_date', 'end_date']) ? '' : 'hidden' }} px-5 py-3 border-b border-gray-100 bg-green-50/30">
                            ...
                            <div>
                                ...
                                <select name="metode_penyaluran"
                                    ...
                                    <option value="transfer"
                                        {{ request('metode_penyaluran') == 'transfer' ? 'selected' : '' }}>Transfer
                                    </option>
                                    <option value="barang"
                                        {{ request('metode_penyaluran') == 'barang' ? 'selected' : '' }}>Barang</option>
                                </select>
                            </div>
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-gray-200 bg-gray-50/50">
                                ...
                                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 hidden lg:table-cell">
                                    ALAMAT</th>
                                <th class="px-6 py-4 text-center text-xs font-semibold text-gray-500">TOTAL TRANSAKSI</th>
                                <th class="px-6 py-4 text-center text-xs font-semibold text-gray-500 hidden lg:table-cell">
                                    TOTAL NOMINAL</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($lembagas as $lembaga)
                                @php
                                    $penyaluranArray = collect($lembaga->transaksiPenyaluran ?? [])
                                        ->map(function ($t) {
                                            $statusMap = [
                                                'disalurkan' => [
                                                    'label' => 'Disalurkan',
                                            ...
                                            // PERBAIKAN: Pastikan nama mustahik diambil dengan benar
                                            $mustahikNama = '-';
                                            if ($t->mustahik && isset($t->mustahik->nama_lengkap)) {
                                                $mustahikNama = $t->mustahik->nama_lengkap;
                                            } elseif ($t->mustahik && isset($t->mustahik->nama)) {
                                                $mustahikNama = $t->mustahik->nama;
                                            } elseif (isset($t->mustahik_nama)) {
                                                $mustahikNama = $t->mustahik_nama;
                                            }
                                            ...
                                            return [
                                                ...
                                                'mustahik_nama' => $mustahikNama,
                                                'initial' => strtoupper(substr($mustahikNama, 0, 1)),
                                    ...
                                    <td colspan="4" class="px-6 py-4 bg-gray-50/30">
                                        <div class="space-y-3">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <h3 class="text-sm font-semibold text-gray-800">Daftar Transaksi Penyaluran
                                                    — {{ $lembaga->nama }}</h3>
                                            </div>

                                            @if (empty($penyaluranArray))
                                                <div
                                                    class="text-center py-8 text-sm text-gray-400 bg-white rounded-xl border">
                                                    Belum ada data transaksi penyaluran</div>
                                            @else
                                                        ...
                                                        <thead class="bg-gray-50">
                                                            <tr>
                                                                ...
                                                                <th
                                                                    ...
                                                                    MUSTAHIK</th>
                                                                <th
                                                                    class="px-4 py-3 text-left text-xs font-semibold text-gray-500 hidden sm:table-cell">
                                                                    METODE</th>
                                                                <th
                                                ...
                                                <div
                                                    class="bg-gray-50 border border-gray-100 px-4 py-2.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-lg">
                                                    <span id="penyaluran-info-{{ $lembaga->id }}"
                                                        class="text-xs text-gray-500"></span>
                                                    <div id="penyaluran-pagination-{{ $lembaga->id }}"
                                                        class="flex items-center justify-center gap-1"></div>
                                                </div>
                            ...
                            $penyaluranArray = collect($lembaga->transaksiPenyaluran ?? [])
                                ->map(function ($t) {
                                    ...
                                    return [
                                        ...
                                        'initial' => strtoupper(
                                            substr($t->mustahik->nama ?? ($t->mustahik_nama ?? 'P'), 0, 1),
                                        ),
                                        'metode' => $t->metode_penyaluran ?? '-',
                                        'jumlah' => $t->jumlah ?? 0,
                                        'status' => $t->status ?? 'draft',
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div>
                                        <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">
                                            Daftar Transaksi Penyaluran</h4>
                                        @if (empty($penyaluranArray))
                                        ...
                                        @else
                                            ...
                                            <div class="mt-3 flex justify-center"
                                                id="mobile-pagination-{{ $lembaga->id }}"></div>
                                        @endif
                                    </div>
                                </div>
                            </div>
            ...
            @else
                <div class="py-16 text-center">
                    <div
                        class="w-20 h-20 bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                    ...
                    @if (request('q') ||
                            request('status') ||
                            request('lembaga_id') ||
                            request('metode_penyaluran') ||
                            request('start_date') ||
                            request('end_date'))
                        <p class="text-sm text-gray-500 mb-2">Tidak ada hasil untuk filter yang dipilih</p>
                        <a href="{{ route('superadmin.transaksi-penyaluran.index') }}"
                            class="text-sm text-green-600 hover:text-green-700">Reset semua filter</a>
                    @else
    ...
    <script>
        ...
        function renderDesktopPenyaluran(lembagaId, slice, start, end, total) {
            ...
            if (tbody) {
                if (slice.length === 0) {
                    tbody.innerHTML =
                        `<tr><td colspan="5" class="px-4 py-8 text-center text-sm text-gray-400">Tidak ada data transaksi penyaluran</td></tr>`;
                } else {
            ...
            if (info) info.textContent = total > 0 ? `Menampilkan ${start + 1}–${end} dari ${total} transaksi` :
                'Tidak ada data';
        ...
        function buildPagination(lembagaId, current, total) {
            ...
            let html = '';
            if (current > 1) html +=
                `<button type="button" onclick="renderPenyaluranPage(${lembagaId}, ${current - 1})" class="${btnNormal}">‹</button>`;
            ...
            range.forEach(p => {
                if (p === '...') html += `<span class="${btnBase} text-gray-400">…</span>`;
                    ...
                    `<button type="button" onclick="renderPenyaluranPage(${lembagaId}, ${p})" class="${p === current ? btnActive : btnNormal}">${p}</button>`;
            });
                ...
                row.addEventListener('click', function(e) {
                    ...
                    if (targetRow) {
                        ...
                        if (isHidden) {
                            ...
                            if ((window.penyaluranData?.[lembagaId] || window.penyaluranDataMobile
                                    ?.[lembagaId]) && !penyaluranPages[lembagaId]) {
