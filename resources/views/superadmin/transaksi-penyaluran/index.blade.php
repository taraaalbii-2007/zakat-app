        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-xl overflow-hidden transition-all duration-300">
            ...
            <div class="px-5 py-4 border-b border-gray-100">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        <h1 class="text-base font-semibold text-gray-800">Transaksi Penyaluran Semua Lembaga</h1>
                        <p class="text-xs text-gray-500 mt-0.5">Kelola dan konfigurasi transaksi penyaluran dari seluruh lembaga</p>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-2">
                        ...
                        <button type="button" id="filterButton"
                            ...
                            {{ request()->hasAny(['q', 'status', 'lembaga_id', 'metode_penyaluran', 'start_date', 'end_date']) ? 'bg-green-50' : '' }}">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div class="hidden md:flex items-center gap-4">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs font-semibold text-gray-700">{{ number_format($totalDraft) }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <div class="w-2 h-2 rounded-full bg-green-500"></div>
                            <span class="text-xs text-gray-500">Disalurkan:</span>
                            <span class="text-xs font-semibold text-gray-700">{{ number_format($totalDisalurkan) }}</span>
                        </div>
                        <div class="flex items-center gap-2">
            ...
            <!-- Filter Panel -->
            <div id="filterPanel" class="{{ request()->hasAny(['q', 'status', 'lembaga_id', 'metode_penyaluran', 'start_date', 'end_date']) ? '' : 'hidden' }} px-5 py-3 border-b border-gray-100 bg-green-50/30">
                <form method="GET" action="{{ route('superadmin.transaksi-penyaluran.index') }}" id="filter-form">
                    <div class="space-y-3">
                ...
                <div class="px-5 py-2.5 border-b border-gray-100">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        @if(request('metode_penyaluran'))
                            <div class="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-green-50 text-green-700 text-xs rounded-lg border border-green-200">
                                Metode: {{ request('metode_penyaluran') }}
                                <button onclick="removeFilter('metode_penyaluran')" class="hover:text-green-900 ml-1">×</button>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="hidden md:block overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tbody>
                            @foreach ($lembagas as $lembaga)
                                @php
                                    $penyaluranArray = collect($lembaga->transaksiPenyaluran ?? [])->map(function($t) {
                                        $statusMap = [
                                            'disalurkan' => ['label' => 'Disalurkan', 'bg' => 'bg-green-100 text-green-800', 'dot' => 'bg-green-500'],
                                            'disetujui' => ['label' => 'Disetujui', 'bg' => 'bg-blue-100 text-blue-800', 'dot' => 'bg-blue-500'],
                                            'draft' => ['label' => 'Draft', 'bg' => 'bg-yellow-100 text-yellow-800', 'dot' => 'bg-yellow-500'],
                                            'dibatalkan' => ['label' => 'Dibatalkan', 'bg' => 'bg-red-100 text-red-800', 'dot' => 'bg-red-500'],
                                        ];
                                        $status = $statusMap[$t->status] ?? ['label' => $t->status, 'bg' => 'bg-gray-100 text-gray-600', 'dot' => 'bg-gray-500'];
                                        
                                        return [
                                            'id' => $t->id,
                                            'no_transaksi' => $t->no_transaksi ?? '-',
                                            'tanggal' => optional($t->tanggal_penyaluran)->format('d M Y') ?? '-',
                                            'mustahik_nama' => $t->mustahik->nama ?? $t->mustahik_nama ?? '-',
                                            'initial' => strtoupper(substr($t->mustahik->nama ?? $t->mustahik_nama ?? 'P', 0, 1)),
                                            'metode' => $t->metode_penyaluran ?? '-',
                                            'jumlah' => $t->jumlah ?? 0,
                                    ...
                                    $totalNominalLembaga = collect($penyaluranArray)->sum('jumlah');
                                @endphp
                                ...
                                <tr class="border-b border-gray-100 hover:bg-green-50/20 cursor-pointer expandable-row"
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-green-50 text-green-700 border border-green-200">
                                            {{ count($penyaluranArray) }} Transaksi
                                        </span>
                                    </td>
                                    ...
                                    <td colspan="4" class="px-6 py-4 bg-gray-50/30">
                                        <div class="space-y-3">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <h3 class="text-sm font-semibold text-gray-800">Daftar Transaksi Penyaluran — {{ $lembaga->nama }}</h3>
                                            </div>

                                            @if (empty($penyaluranArray))
                                                <div class="text-center py-8 text-sm text-gray-400 bg-white rounded-xl border">Belum ada data transaksi penyaluran</div>
                                            @else
                                                    ...
                                                    <table class="min-w-full divide-y divide-gray-200">
                                                        <thead class="bg-gray-50">
                                                            <tr>
                                                                ...
                                                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500">MUSTAHIK</th>
                                                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 hidden sm:table-cell">METODE</th>
                                                        ...
                                                        <tbody id="penyaluran-tbody-{{ $lembaga->id }}"></tbody>
                                                    </table>
                                                </div>
                                                <div class="bg-gray-50 border border-gray-100 px-4 py-2.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-lg">
                                                    ...
                                                    <div id="penyaluran-pagination-{{ $lembaga->id }}" class="flex items-center justify-center gap-1"></div>
                                                </div>
                                                
                                                <script>
                                                    if (typeof window.penyaluranData === 'undefined') window.penyaluranData = {};
                                                    window.penyaluranData[{{ $lembaga->id }}] = @json($penyaluranArray);
                                                </script>
                                            @endif
                                        </div>
                                    </td>
                ...
                <div class="block md:hidden divide-y divide-gray-100">
                    @foreach ($lembagas as $lembaga)
                        @php
                            $penyaluranArray = collect($lembaga->transaksiPenyaluran ?? [])->map(function($t) {
                                $statusMap = [
                                    'disalurkan' => ['label' => 'Disalurkan', 'bg' => 'bg-green-100 text-green-800'],
                                    'disetujui' => ['label' => 'Disetujui', 'bg' => 'bg-blue-100 text-blue-800'],
                                    'draft' => ['label' => 'Draft', 'bg' => 'bg-yellow-100 text-yellow-800'],
                                    'dibatalkan' => ['label' => 'Dibatalkan', 'bg' => 'bg-red-100 text-red-800'],
                                ];
                                $status = $statusMap[$t->status] ?? ['label' => $t->status, 'bg' => 'bg-gray-100 text-gray-600'];
                        ...
                        <div class="p-4">
                            ...
                            <div id="detail-mobile-{{ $lembaga->id }}" class="hidden mt-3 pt-3 border-t border-gray-100">
                                <div class="space-y-3">
                                    ...
                                    @endif
                                    <div>
                                        <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Daftar Transaksi Penyaluran</h4>
                                        @if (empty($penyaluranArray))
                                            <p class="text-sm text-gray-400 italic">Belum ada data transaksi penyaluran</p>
                                        @else
                                            <div class="space-y-3" id="mobile-penyaluran-container-{{ $lembaga->id }}">
                                                <!-- Akan diisi JS -->
                                            </div>
                        ...
                        <script>
                            if (typeof window.penyaluranDataMobile === 'undefined') window.penyaluranDataMobile = {};
                            window.penyaluranDataMobile[{{ $lembaga->id }}] = @json($penyaluranArray);
                        </script>
                    @endforeach
                </div>
            @else
                <div class="py-16 text-center">
                    ...
                    @if(request('q') || request('status') || request('lembaga_id') || request('metode_penyaluran') || request('start_date') || request('end_date'))
                        <p class="text-sm text-gray-500 mb-2">Tidak ada hasil untuk filter yang dipilih</p>
                        <a href="{{ route('superadmin.transaksi-penyaluran.index') }}" class="text-sm text-green-600 hover:text-green-700">Reset semua filter</a>
                    @else
...
function renderPenyaluranPage(lembagaId, page) {
    ...
    const totalPages = Math.ceil(total / PENYALURAN_PER_PAGE) || 1;
...
function renderDesktopPenyaluran(lembagaId, slice, start, end, total) {
    ...
    if (tbody) {
        if (slice.length === 0) {
            tbody.innerHTML = `<tr><td colspan="5" class="px-4 py-8 text-center text-sm text-gray-400">Tidak ada data transaksi penyaluran</td></tr>`;
        } else {
            tbody.innerHTML = slice.map(t => {
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3">
                        ...
                        <div class="text-xs text-gray-400">${escapeHtml(t.tanggal)}</div>
                     </td>
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-2">
...
function renderMobilePenyaluran(lembagaId, page, totalPages) {
    const data = window.penyaluranDataMobile?.[lembagaId] ?? [];
    const total = data.length;
    const start = (page - 1) * PENYALURAN_PER_PAGE;
    const slice = data.slice(start, start + PENYALURAN_PER_PAGE);
    ...
    if (container) {
        if (slice.length === 0) {
            container.innerHTML = `<p class="text-sm text-gray-400 italic">Tidak ada data transaksi penyaluran</p>`;
        } else {
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            <div class="flex items-start justify-between gap-2 flex-wrap">
                                <div>
                                    <p class="text-sm font-medium text-gray-900">${escapeHtml(t.mustahik_nama)}</p>
                            ...
                            <div class="flex flex-wrap items-center justify-between gap-2 mt-2">
                                ...
                                <p class="text-xs text-gray-400">${escapeHtml(t.tanggal)}</p>
                            </div>
                        </div>
                    </div>
                </div>`;
...
function buildPagination(lembagaId, current, total) {
    ...
    let html = '';
    if (current > 1) html += `<button type="button" onclick="renderPenyaluranPage(${lembagaId}, ${current - 1})" class="${btnNormal}">‹</button>`;
    ...
    range.forEach(p => {
        ...
        else html += `<button type="button" onclick="renderPenyaluranPage(${lembagaId}, ${p})" class="${p === current ? btnActive : btnNormal}">${p}</button>`;
    });
    
    if (current < total) html += `<button type="button" onclick="renderPenyaluranPage(${lembagaId}, ${current + 1})" class="${btnNormal}">›</button>`;
    else html += `<button disabled class="${btnDisabled}">›</button>`;
    return html;
}
        ...
        row.addEventListener('click', function(e) {
            ...
            if (targetRow) {
                ...
                if (isHidden) {
                    ...
                    if ((window.penyaluranData?.[lembagaId] || window.penyaluranDataMobile?.[lembagaId]) && !penyaluranPages[lembagaId]) {
