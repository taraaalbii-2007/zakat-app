        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-card border border-gray-100 overflow-hidden animate-slide-up">
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h2 class="text-base sm:text-lg font-semibold text-gray-900">Edit Data Lembaga</h2>
                        <p class="text-xs sm:text-sm text-gray-500 mt-1">Edit data lembaga "{{ $lembaga->nama }}"</p>
                    </div>
                    <div class="mt-2 sm:mt-0">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $lembaga->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            {{ $lembaga->is_active ? 'Aktif' : 'Nonaktif' }}
                        </span>
                        <span class="ml-2 inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                            Kode: {{ $lembaga->kode_lembaga }}
                        </span>
                    </div>
                </div>
            </div>
            <form action="{{ route('lembaga.update', $lembaga) }}" method="POST" enctype="multipart/form-data" class="p-4 sm:p-6">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    ...
                    <div class="border-b border-gray-200 pb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="admin_nama" id="admin_nama" value="{{ old('admin_nama', $lembaga->admin_nama) }}"
                            ...
                            <div>
                                ...
                                <input type="text" name="admin_telepon" id="admin_telepon" value="{{ old('admin_telepon', $lembaga->admin_telepon) }}"
                            ...
                            <div>
                                ...
                                <input type="email" name="admin_email" id="admin_email" value="{{ old('admin_email', $lembaga->admin_email) }}"
                                ...
                                @if($lembaga->admin_foto)
                                    <div class="mb-3">
                                        ...
                                        <div class="relative w-24 h-24 border border-gray-300 rounded-xl overflow-hidden">
                                            <img src="{{ Storage::url($lembaga->admin_foto) }}" alt="Foto Admin" class="w-full h-full object-cover">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="sejarah" id="sejarah" rows="4"
                                    ...
                                    class="block w-full px-3 py-2 text-sm sm:text-base border border-gray-300 rounded-xl focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary @error('sejarah') border-red-500 @enderror">{{ old('sejarah', $lembaga->sejarah) }}</textarea>
                                @error('sejarah')
                                    <p class="mt-1 text-xs sm:text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="pendiri" id="pendiri" value="{{ old('pendiri', $lembaga->pendiri) }}"
                    ...
                    <div class="pb-6">
                        ...
                        {{-- Kode Lembaga --}}
                        <div class="mb-4">
                            <label for="kode_lembaga" class="block text-sm font-medium text-gray-700 mb-2">
                                Kode Lembaga <span class="text-red-500">*</span>
                            </label>
                            <input type="text" name="kode_lembaga" id="kode_lembaga" value="{{ old('kode_lembaga', $lembaga->kode_lembaga) }}"
                                placeholder="MASJID-001"
                                maxlength="50"
                                class="block w-full px-3 py-2 text-sm sm:text-base border border-gray-300 rounded-xl focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary @error('kode_lembaga') border-red-500 @enderror">
                            <p class="mt-1 text-xs text-gray-500">Kode unik untuk identifikasi lembaga</p>
                            @error('kode_lembaga')
                                <p class="mt-1 text-xs sm:text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="nama" id="nama" value="{{ old('nama', $lembaga->nama) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="alamat" id="alamat" rows="3"
                                ...
                                class="block w-full px-3 py-2 text-sm sm:text-base border border-gray-300 rounded-xl focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary @error('alamat') border-red-500 @enderror">{{ old('alamat', $lembaga->alamat) }}</textarea>
                            @error('alamat')
                                <p class="mt-1 text-xs sm:text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                ...
                                <select name="provinsi_kode" id="provinsi_kode"
                                    ...
                                    @foreach($provinces as $province)
                                        <option value="{{ $province->code }}" 
                                            {{ (old('provinsi_kode', $lembaga->provinsi_kode) == $province->code) ? 'selected' : '' }}>
                                            {{ $province->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="kota_kode" id="kota_kode"
                                    ...
                                    {{ !$lembaga->provinsi_kode ? 'disabled' : '' }}>
                                    <option value="">Pilih Kota/Kabupaten</option>
                                    @foreach($cities as $city)
                                        <option value="{{ $city->code }}"
                                            {{ (old('kota_kode', $lembaga->kota_kode) == $city->code) ? 'selected' : '' }}>
                                            {{ $city->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="kecamatan_kode" id="kecamatan_kode"
                                    ...
                                    {{ !$lembaga->kota_kode ? 'disabled' : '' }}>
                                    <option value="">Pilih Kecamatan</option>
                                    @foreach($districts as $district)
                                        <option value="{{ $district->code }}"
                                            {{ (old('kecamatan_kode', $lembaga->kecamatan_kode) == $district->code) ? 'selected' : '' }}>
                                            {{ $district->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="kelurahan_kode" id="kelurahan_kode"
                                    ...
                                    {{ !$lembaga->kecamatan_kode ? 'disabled' : '' }}>
                                    <option value="">Pilih Kelurahan/Desa</option>
                                    @foreach($villages as $village)
                                        <option value="{{ $village->code }}"
                                            {{ (old('kelurahan_kode', $lembaga->kelurahan_kode) == $village->code) ? 'selected' : '' }}>
                                            {{ $village->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                            <div>
                                ...
                                <input type="text" name="kode_pos" id="kode_pos" value="{{ old('kode_pos', $lembaga->kode_pos) }}"
                                    placeholder="12345"
                                    maxlength="10"
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="deskripsi" id="deskripsi" rows="4"
                                ...
                                class="block w-full px-3 py-2 text-sm sm:text-base border border-gray-300 rounded-xl focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $lembaga->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <p class="mt-1 text-xs sm:text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>

                        {{-- Foto Lembaga (Existing + New) --}}
                        <div class="mb-4">
                            ...
                            @if($fotoCount > 0)
                                <div class="mb-4">
                                    ...
                                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-{{ min(\App\Models\Lembaga::MAX_FOTO, 5) }} gap-3" id="existing-fotos-container">
                                        @foreach($fotos as $index => $fotoPath)
                                            <div class="foto-preview-item existing-foto" data-index="{{ $index }}">
                                                <img src="{{ Storage::url($fotoPath) }}" alt="Foto Lembaga {{ $index + 1 }}">
                            ...
                            {{-- New Photos Input --}}
                            @php
                                $remainingSlots = $lembaga->getRemainingFotoSlots();
                            @endphp
                            ...
                            @if($remainingSlots > 0)
                                <div class="mt-4">
                                    ...
                                    {{-- New Photos Preview --}}
                                    <div id="new-foto-preview-container" class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-{{ min(\App\Models\Lembaga::MAX_FOTO, 5) }} gap-3">
                            ...
                            @else
                                <div class="p-3 bg-yellow-50 border border-yellow-200 rounded-xl">
                                    <p class="text-sm text-yellow-700">
                                        Sudah mencapai batas maksimal {{ \App\Models\Lembaga::MAX_FOTO }} foto. 
...
<script>
    ...
    function initForm() {
        if (window.innerWidth >= 768) {
            document.getElementById('kode_lembaga')?.focus();
        }
    ...
    function handleFotoSelection(event) {
        ...
        const remainingSlots = {{ $lembaga->getRemainingFotoSlots() }};
    ...
    function loadOldWilayahValues() {
        const oldProvinsiKode = '{{ old("provinsi_kode", $lembaga->provinsi_kode) }}';
        const oldKotaKode = '{{ old("kota_kode", $lembaga->kota_kode) }}';
        const oldKecamatanKode = '{{ old("kecamatan_kode", $lembaga->kecamatan_kode) }}';
        const oldKelurahanKode = '{{ old("kelurahan_kode", $lembaga->kelurahan_kode) }}';
