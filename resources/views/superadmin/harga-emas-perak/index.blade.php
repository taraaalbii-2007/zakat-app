...
@section('content')
        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-card border border-gray-100 overflow-hidden animate-slide-up">
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                    <div>
                        <h2 class="text-base sm:text-lg font-semibold text-gray-900">Harga Emas & Perak</h2>
                        <p class="text-xs sm:text-sm text-gray-500 mt-1">Total: {{ $hargaEmasPerak->total() }} Data</p>
                    </div>
                    <div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
                        ...
                        <div id="search-container" class="transition-all duration-300"
                            ...
                            <form method="GET" action="{{ route('harga-emas-perak.index') }}" id="search-form"
                                ...
                                <div class="flex items-center">
                                    <div class="relative flex-1">
                                        ...
                                        <input type="search" name="search" value="{{ request('search') }}"
                                            id="search-input" placeholder="Cari sumber atau keterangan..."
            ...
            {{-- Filter Panel --}}
            <div id="filter-panel"
                class="{{ request('status') || request('tanggal') || request('sumber') || request('sort_by') ? '' : 'hidden' }} px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 border-b border-gray-200">
                <form method="GET" action="{{ route('harga-emas-perak.index') }}" id="filter-form">
                    @if (request('search'))
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-5 gap-3 sm:gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal" id="filter-tanggal" value="{{ request('tanggal') }}"
                                class="block w-full px-3 py-2 text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">Sumber</label>
                            <select name="sumber" id="filter-sumber"
                                class="block w-full px-3 py-2 text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all">
                                <option value="">Semua Sumber</option>
                                @foreach ($sumberList as $sumber)
                                    @if ($sumber)
                                        <option value="{{ $sumber }}"
                                            {{ request('sumber') == $sumber ? 'selected' : '' }}>{{ $sumber }}
                                        </option>
                                    @endif
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="sort_by" id="filter-sort-by"
                                class="block w-full px-3 py-2 text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all">
                                <option value="tanggal" {{ request('sort_by') == 'tanggal' ? 'selected' : '' }}>Tanggal
                                </option>
                    ...
                    @if (request('status') || request('tanggal') || request('sumber') || request('sort_by') || request('sort_order'))
                        <div class="mt-3 flex justify-end">
                            <a href="{{ route('harga-emas-perak.index', request('search') ? ['search' => request('search')] : []) }}"
            ...
            @if ($hargaEmasPerak->count() > 0)
                {{-- Info Filter Aktif --}}
                @if (request('status') || request('tanggal') || request('sumber') || request('search'))
                            ...
                            <div class="flex items-center flex-wrap gap-2">
                                ...
                                @if (request('tanggal'))
                                    <span
                                        ...
                                        <button type="button" onclick="removeFilter('tanggal')"
                                            class="ml-1.5 text-blue-600 hover:text-blue-800">
                                            ×
                                        </button>
                                    </span>
                                ...
                                @if (request('sumber'))
                                    <span
                                        ...
                                        <button type="button" onclick="removeFilter('sumber')"
                                            class="ml-1.5 text-blue-600 hover:text-blue-800">
                                            ×
                                        </button>
                                    </span>
                                ...
                                @if (request('sort_by') && request('sort_by') != 'tanggal')
                                    <span
                                        ...
                                        <button type="button" onclick="removeFilter('sort_by')"
                                            class="ml-1.5 text-blue-600 hover:text-blue-800">
                                            ×
                                        </button>
                                    </span>
                            ...
                            @foreach ($hargaEmasPerak as $harga)
                                ...
                                <tr class="hover:bg-gray-50 transition-colors cursor-pointer expandable-row"
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs text-gray-500 mt-0.5">per gram</div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-medium text-gray-900">
                                ...
                                <tr id="detail-{{ $harga->uuid }}" class="hidden expandable-content">
                                        ...
                                        <div class="bg-gray-50 border-y border-gray-100">
                                            <div class="px-6 py-4">
                                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="space-y-3">
                                                            ...
                                                            <div class="flex items-start">
                                                                ...
                                                                <div>
                                                                    <p class="text-xs text-gray-500">Sumber Data</p>
                                                                    <p class="text-sm font-medium text-gray-900">
                                                                        {{ $harga->sumber ?? 'Tidak ada sumber' }}</p>
                                                                </div>
                                                            </div>

                                                            <div class="flex items-start">
                                                                <svg class="w-4 h-4 {{ $harga->is_active ? 'text-green-500' : 'text-gray-400' }} mt-0.5 mr-2 flex-shrink-0"
                                                                    fill="none" stroke="currentColor"
                                                                    viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                                        stroke-width="2"
                                                                ...
                                                                <div>
                                                                    ...
                                                                    @if ($harga->is_active)
                                                                        <p class="text-sm font-medium text-green-600">Aktif
                                                                        </p>
                                                                    @else
                                                    ...
                                                    {{-- Kolom 2: Harga --}}
                                                    <div>
                                                        <h4 class="text-sm font-medium text-gray-900 mb-3">Detail Harga
                                                        </h4>
                                                        <div class="space-y-3">
                                                            <div class="flex items-start">
                                                                <svg class="w-4 h-4 text-yellow-500 mt-0.5 mr-2 flex-shrink-0"
                                                                    fill="none" stroke="currentColor"
                                                                    viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                                        stroke-width="2"
                                                                ...
                                                                <div>
                                                                    <p class="text-xs text-gray-500">Harga Emas</p>
                                                                    <p class="text-sm font-medium text-gray-900">Rp
                                                                        {{ number_format($harga->harga_emas_pergram, 0, ',', '.') }}/gram
                                                                    </p>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div class="flex items-start">
                                                                ...
                                                                <div>
                                                                    <p class="text-xs text-gray-500">Harga Perak</p>
                                                                    <p class="text-sm font-medium text-gray-900">Rp
                                                                        {{ number_format($harga->harga_perak_pergram, 0, ',', '.') }}/gram
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    ...
                                                    {{-- Kolom 3: Keterangan & Metadata --}}
                                                    <div>
                                                        <h4 class="text-sm font-medium text-gray-900 mb-3">Keterangan</h4>
                                                        @if ($harga->keterangan)
                                                            <p class="text-sm text-gray-600">{{ $harga->keterangan }}</p>
                                                        @else
                                                            <p class="text-sm text-gray-400 italic">Tidak ada keterangan
                                                            </p>
                                                        @endif

                                                        <div class="mt-4 pt-4 border-t border-gray-200">
                                                            <div class="text-xs text-gray-500 space-y-1">
                                                                <div>Dibuat: {{ $harga->created_at->format('d/m/Y H:i') }}
                                                                </div>
                                                                @if ($harga->updated_at != $harga->created_at)
                                                                    <div>Diperbarui:
                                                                        {{ $harga->updated_at->format('d/m/Y H:i') }}</div>
                                                                @endif
                                                            </div>
                                                        </div>
                                                    </div>
                                                ...
                                                <div
                                                    ...
                                                    <div class="flex gap-2">
                                                        ...
                                                        <form
                                                            action="{{ route('harga-emas-perak.toggle-status', $harga->uuid) }}"
                                                            method="POST" class="inline">
                                                            @csrf
                                                            @method('PATCH')
                                                            <button type="submit"
                                                                class="inline-flex items-center px-3 py-1.5 {{ $harga->is_active ? 'bg-yellow-100 hover:bg-yellow-200 text-yellow-700' : 'bg-green-100 hover:bg-green-200 text-green-700' }} text-xs font-medium rounded-lg transition-all">
                                                                <svg class="w-4 h-4 mr-1.5" fill="none"
                                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                                    @if ($harga->is_active)
                ...
                <div class="md:hidden divide-y divide-gray-200">
                    @foreach ($hargaEmasPerak as $harga)
                        <div class="expandable-card">
                                    ...
                                    <div class="flex items-center flex-1 min-w-0">
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <div class="flex items-center mt-1">
                                                <span class="text-xs font-medium text-gray-700">Emas: Rp
                                                    {{ number_format($harga->harga_emas_pergram, 0, ',', '.') }}</span>
                                                <span class="text-xs text-gray-500 mx-2">•</span>
                                                <span class="text-xs font-medium text-gray-700">Perak: Rp
                                                    {{ number_format($harga->harga_perak_pergram, 0, ',', '.') }}</span>
                                            </div>
                                            @if ($harga->sumber)
                                                <div class="text-xs text-gray-500 mt-1 truncate">Sumber:
                                                    {{ $harga->sumber }}</div>
                                            @endif
                                        </div>
                                    </div>
                            ...
                            <div id="detail-mobile-{{ $harga->uuid }}" class="hidden expandable-content-mobile">
                                <div class="bg-gray-50 px-4 py-3 border-t border-gray-100">
                                    <div class="space-y-4">
                                        {{-- Informasi Harga --}}
                                        <div>
                                            <h4 class="text-sm font-medium text-gray-900 mb-2">Detail Harga</h4>
                                            <div class="space-y-2">
                                                <div class="flex items-center text-sm">
                                                    <svg class="w-4 h-4 text-yellow-500 mr-2 flex-shrink-0" fill="none"
                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                                                            d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                                                    </svg>
                                                    <span class="text-gray-900">Emas: Rp
                                                ...
                                                <div class="flex items-center text-sm">
                                                    ...
                                                    <span class="text-gray-900">Perak: Rp
                                        ...
                                        @if ($harga->sumber)
                                            <div>
                                                <h4 class="text-sm font-medium text-gray-900 mb-2">Sumber Data</h4>
                                                <div class="flex items-center text-sm">
                                                    <svg class="w-4 h-4 text-gray-400 mr-2 flex-shrink-0" fill="none"
                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                    ...
                                                    <span class="text-gray-900">{{ $harga->sumber }}</span>
                                                </div>
                                            </div>
                                        @endif
                                        ...
                                        @if ($harga->keterangan)
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-600">{{ Str::limit($harga->keterangan, 100) }}
                                                </p>
                                            </div>
                                        @endif
                                        ...
                                        <div class="pt-3 border-t border-gray-200">
                                            <div class="flex gap-2">
                                                ...
                                                <form action="{{ route('harga-emas-perak.toggle-status', $harga->uuid) }}"
                                                    ...
                                                    @method('PATCH')
                                                    <button type="submit"
                                                        class="w-full inline-flex items-center justify-center px-3 py-2 {{ $harga->is_active ? 'bg-yellow-100 hover:bg-yellow-200 text-yellow-700' : 'bg-green-100 hover:bg-green-200 text-green-700' }} text-xs font-medium rounded-lg transition-all">
                                                        <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            @if ($harga->is_active)
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                        ...
                                                        {{ $harga->is_active ? 'Nonaktif' : 'Aktif' }}
                                                    </button>
                                                </form>
                                            </div>
                                        </div>
            ...
            @else
                <div class="p-8 sm:p-12 text-center">
                    ...
                    @if (request('search') || request('status') || request('tanggal') || request('sumber') || request('sort_by'))
                        <h3 class="text-base sm:text-lg font-medium text-gray-900 mb-2">Data Tidak Ditemukan</h3>
                        <p class="text-sm text-gray-500 mb-6">
                            @if (request('search'))
                                Tidak ada data harga emas/perak yang cocok dengan "{{ request('search') }}"
                            @else
                    ...
                    @else
                        <h3 class="text-base sm:text-lg font-medium text-gray-900 mb-2">Belum Ada Data Harga</h3>
                        <p class="text-sm text-gray-500 mb-6">Mulai tambahkan data harga emas dan perak untuk sistem zakat.
                        </p>
                        <a href="{{ route('harga-emas-perak.create') }}"
    ...
    <div id="dropdown-container" class="fixed hidden z-50">
        <div class="w-44 sm:w-48 rounded-xl shadow-lg bg-white ring-1 ring-black ring-opacity-5">
            <div class="py-1">
                ...
                <form id="dropdown-toggle-form" method="POST" class="w-full">
                    @csrf
                    @method('PATCH')
                    <button type="submit"
                        ...
                        <span id="dropdown-toggle-text">Toggle Status</span>
                    </button>
                </form>
    ...
    <div id="delete-modal"
        ...
        <div class="p-4 sm:p-6 border border-gray-200 w-full max-w-sm shadow-lg rounded-xl sm:rounded-2xl bg-white">
            ...
            <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-1.5 sm:mb-2 text-center">Hapus Data Harga</h3>
            <p class="text-xs sm:text-sm text-gray-500 mb-1 text-center">
                Apakah Anda yakin ingin menghapus data harga untuk tanggal
                "<span id="modal-tanggal" class="font-semibold text-gray-700"></span>"?
            </p>
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const toggleForm = document.getElementById('dropdown-toggle-form');
            const toggleText = document.getElementById('dropdown-toggle-text');
            ...
            document.addEventListener('click', function(e) {
                ...
                if (toggle) {
                    ...
                    const isActive = toggle.getAttribute('data-active') === '1';
...
requestAnimationFrame(() => {
    ...
    dropdownContainer.style.visibility = '';
            ...
            deleteBtn.addEventListener('click', function(e) {
                ...
                const tanggal = currentDropdownData?.tanggal;
