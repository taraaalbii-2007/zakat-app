...
@section('content')
    <div class="space-y-5">
        ...
        <div class="bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-soft transition-all duration-300">
            ...
            <div class="px-4 sm:px-6 py-4 sm:py-5 border-b border-neutral-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p class="text-xs sm:text-sm text-neutral-500 mt-0.5 sm:mt-1">Data referensi harga untuk perhitungan nisab zakat</p>
                    </div>
            ...
            <div id="filter-panel" class="{{ request('status') || request('tanggal') || request('sumber') || request('sort_by') || request('sort_order') ? '' : 'hidden' }} px-4 sm:px-6 py-4 border-b border-neutral-200 bg-neutral-50/30">
                <div class="flex flex-wrap items-end gap-4">
                    ...
                    <div class="min-w-[140px] flex-1 sm:flex-none">
                        ...
                        <input type="date" id="filter-tanggal" value="{{ request('tanggal') }}"
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            @if (request('tanggal'))
                                <span class="inline-flex items-center gap-1.5 px-2 py-0.5 bg-primary-50 text-primary-700 text-xs rounded-full">
                                    ...
                                    <button onclick="removeFilter('tanggal')" class="hover:text-primary-900 transition-colors">×</button>
                                </span>
                            @endif
                            @if (request('sumber'))
                                <span class="inline-flex items-center gap-1.5 px-2 py-0.5 bg-primary-50 text-primary-700 text-xs rounded-full">
                                    Sumber: {{ request('sumber') }}
                                    <button onclick="removeFilter('sumber')" class="hover:text-primary-900 transition-colors">×</button>
                                </span>
                            @endif
                        </div>
                    @endif
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b-2 border-neutral-200 bg-neutral-50">
                                ...
                                <th class="px-6 py-4 text-left text-sm font-semibold text-neutral-700">TANGGAL & STATUS</th>
                        ...
                        <tbody>
                            @foreach ($hargaEmasPerak as $harga)
                                ...
                                <tr class="border-b border-neutral-100 hover:bg-primary-50/20 transition-all duration-200 group cursor-pointer expandable-row"
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="text-sm {{ $harga->sumber ? 'text-neutral-800' : 'text-neutral-400' }}">
                                            {{ $harga->sumber ?? '-' }}
                                        </span>
                                    </td>
                                ...
                                <tr id="detail-{{ $harga->uuid }}" class="hidden border-b border-neutral-100 expandable-content">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                            ...
                                            <div class="space-y-2">
                                                ...
                                                <div class="text-xs text-neutral-500 space-y-0.5">
                                                    <div>Tanggal: <span class="font-medium text-neutral-700">{{ $harga->tanggal->format('d F Y') }}</span></div>
                                                    <div>Sumber: <span class="font-medium text-neutral-700">{{ $harga->sumber ?? 'Tidak ada sumber' }}</span></div>
                                                    <div>Status:
                                                        <span class="font-medium {{ $harga->is_active ? 'text-green-600' : 'text-neutral-600' }}">
                                            ...
                                            <!-- Kolom 2: Keterangan -->
                                            <div>
                                                <h4 class="text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">Keterangan</h4>
                                                @if ($harga->keterangan)
                                            ...
                                            <div class="text-xs text-neutral-400 space-y-0.5">
                                                <h4 class="text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">Riwayat</h4>
                                    ...
                                    <td class="px-6 py-4 align-top bg-neutral-50/50 text-center">
                                        <div class="flex flex-col gap-2 items-center">
                                            ...
                                            <form action="{{ route('harga-emas-perak.toggle-status', $harga->uuid) }}" method="POST">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
                ...
                <div class="block md:hidden divide-y divide-neutral-100">
                    @foreach ($hargaEmasPerak as $harga)
                        <div class="p-4 hover:bg-primary-50/20 transition-all duration-200">
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="flex flex-wrap items-center gap-1.5 mt-2">
                                            ...
                                            @else
                                                <span class="inline-flex items-center px-2 py-0.5 bg-neutral-100 text-neutral-500 text-xs rounded-full">
                                                    Nonaktif
                                                </span>
                                            @endif
                                            <span class="text-xs text-neutral-500">Emas: Rp {{ number_format($harga->harga_emas_pergram, 0, ',', '.') }}</span>
                                            <span class="text-xs text-neutral-300">•</span>
                                            <span class="text-xs text-neutral-500">Perak: Rp {{ number_format($harga->harga_perak_pergram, 0, ',', '.') }}</span>
                                        </div>
                                    </div>
                            ...
                            <div id="detail-mobile-{{ $harga->uuid }}" class="hidden mt-3 pt-3 border-t border-neutral-100">
                                <div class="space-y-4">
                                    <!-- Detail Harga -->
                                    <div>
                                        <h4 class="text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">Detail Harga</h4>
                                        <div class="bg-neutral-50 rounded-lg p-3 space-y-2">
                                            <div class="flex justify-between">
                                                <span class="text-xs text-neutral-500">Harga Emas</span>
                                                <span class="text-xs font-semibold text-neutral-800">Rp {{ number_format($harga->harga_emas_pergram, 0, ',', '.') }}/gram</span>
                                            </div>
                                            <div class="flex justify-between">
                                                <span class="text-xs text-neutral-500">Harga Perak</span>
                                                <span class="text-xs font-semibold text-neutral-800">Rp {{ number_format($harga->harga_perak_pergram, 0, ',', '.') }}/gram</span>
                                            </div>
                                            <div class="flex justify-between">
                                                <span class="text-xs text-neutral-500">Sumber</span>
                                                <span class="text-xs font-medium text-neutral-700">{{ $harga->sumber ?? '-' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    @if ($harga->keterangan)
                                        <div>
                                            ...
                                            <div class="bg-neutral-50 rounded-lg p-3">
                                                <p class="text-sm text-neutral-600">{{ Str::limit($harga->keterangan, 100) }}</p>
                                            </div>
                                        </div>
                                    @endif
                ...
                <div class="py-14 text-center animate-fade-in">
                    ...
                    @if (request('search') || request('status') || request('tanggal') || request('sumber'))
                        ...
                        <a href="{{ route('harga-emas-perak.index') }}"
                            class="inline-block mt-3 text-sm text-primary-600 hover:text-primary-700 transition-colors">Reset filter</a>
                    @else
                        <p class="text-sm text-neutral-500">Belum ada data harga emas & perak</p>
                        <a href="{{ route('harga-emas-perak.create') }}"
                            class="inline-block mt-3 text-sm text-primary-600 hover:text-primary-700 transition-colors">Tambah data</a>
                    @endif
                </div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl max-w-sm w-full">
            <div class="p-5">
                ...
                <p class="text-sm text-neutral-500 mb-1 text-center">
                    Yakin ingin menghapus data harga tanggal "<span id="modal-tanggal" class="font-semibold text-neutral-700"></span>"?
                </p>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (applyFilterBtn) {
            applyFilterBtn.addEventListener('click', function() {
                ...
                tanggal ? url.searchParams.set('tanggal', tanggal) : url.searchParams.delete('tanggal');
                sumber ? url.searchParams.set('sumber', sumber) : url.searchParams.delete('sumber');
                (sortBy && sortBy !== 'tanggal') ? url.searchParams.set('sort_by', sortBy) : url.searchParams.delete('sort_by');
                (sortOrder && sortOrder !== 'desc') ? url.searchParams.set('sort_order', sortOrder) : url.searchParams.delete('sort_order');
        ...
        document.querySelectorAll('.expandable-row').forEach(row => {
            row.addEventListener('click', function(e) {
                if (e.target.closest('.dropdown-toggle') || e.target.closest('a') || e.target.closest('button[type="submit"]')) return;
        ...
        document.addEventListener('click', function(e) {
            ...
            if (toggle) {
                ...
                currentDropdownData = { uuid: dropdownUuid, tanggal: tanggal };
