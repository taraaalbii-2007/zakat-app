        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-card border border-gray-100 overflow-hidden animate-slide-up">
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h2 class="text-base sm:text-lg font-semibold text-gray-900">Edit Jenis Zakat</h2>
                        <p class="text-xs sm:text-sm text-gray-500 mt-1">Edit data jenis zakat "{{ $jenisZakat->nama }}"</p>
                    </div>
                </div>
            </div>

            <form action="{{ route('jenis-zakat.update', $jenisZakat->uuid) }}" method="POST" class="p-4 sm:p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama" id="nama" value="{{ old('nama', $jenisZakat->nama) }}"
                        ...
                        <p class="mt-1 text-xs text-gray-500">Masukkan nama jenis zakat</p>
                        @error('nama')
                    ...
                    <div class="p-3 bg-gray-50 border border-gray-200 rounded-xl">
                        <p class="text-xs text-gray-700 font-medium mb-2">Informasi Sistem:</p>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-gray-600">
                            <div>
                                <span class="font-medium">Dibuat:</span> {{ $jenisZakat->created_at->format('d/m/Y H:i') }}
                            </div>
                            <div>
                                <span class="font-medium">Diperbarui:</span> {{ $jenisZakat->updated_at->format('d/m/Y H:i') }}
                            </div>
                        </div>
                    </div>
