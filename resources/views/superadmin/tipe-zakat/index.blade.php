        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-card border border-gray-100 overflow-hidden animate-slide-up">
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                    ...
                    {{-- ACTION BUTTONS: mobile = icon row, desktop = original --}}
                    <div class="flex items-center gap-2 sm:gap-3">
                        {{-- Tambah --}}
                        <a href="{{ route('tipe-zakat.create') }}"
                            class="group inline-flex items-center justify-center px-3 py-2 bg-primary hover:bg-primary-600 text-white text-sm font-medium rounded-lg transition-all shadow-sm flex-1 sm:flex-none">
                            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                            </svg>
                            <span class="ml-2 sm:hidden">Tambah</span>
                            <span class="hidden sm:inline-block sm:ml-2">Tambah Tipe</span>
                        </a>
                        ...
                        {{-- Search --}}
                        <div id="search-container" class="transition-all duration-300 flex-1 sm:flex-none"
                            style="{{ request('search') ? 'min-width: 180px;' : '' }}">
                            <button type="button" onclick="toggleSearch()" id="search-button"
                                class="inline-flex items-center justify-center p-2 sm:px-3 sm:py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-lg transition-all w-full {{ request('search') ? 'hidden' : '' }}">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div class="flex items-center gap-1">
                                    <div class="relative flex-1">
                                        ...
                                        <input type="search" name="search" value="{{ request('search') }}"
                                            id="search-input" placeholder="Cari tipe zakat..."
            ...
            {{-- Filter Panel --}}
            <div id="filter-panel"
                class="{{ request('jenis_zakat_id') || request('requires_haul') || request('sort_by') ? '' : 'hidden' }} px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 border-b border-gray-200">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
                        <div>
                            ...
                            <select name="jenis_zakat_id" id="filter-jenis-zakat"
                                class="block w-full px-2 py-1.5 sm:px-3 sm:py-2 text-xs sm:text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all">
                        ...
                        <div>
                            ...
                            <select name="requires_haul" id="filter-haul"
                                ...
                                <option value="true" {{ request('requires_haul') == 'true' ? 'selected' : '' }}>Perlu Haul</option>
                        ...
                        <div>
                            ...
                            <select name="sort_by" id="filter-sort-by"
                                ...
                                <option value="persentase_zakat"
                                    {{ request('sort_by') == 'persentase_zakat' ? 'selected' : '' }}>Persentase</option>
            ...
            @if ($tipeZakat->count() > 0)
                            ...
                            <div class="flex items-center flex-wrap gap-1.5">
                                ...
                                @if (request('search'))
                                    <span class="inline-flex items-center px-2 py-0.5 bg-blue-100 text-blue-800 text-xs rounded-full">
                                        ...
                                        <button type="button" onclick="removeFilter('search')" class="ml-1 text-blue-600 hover:text-blue-800 leading-none">×</button>
                                    </span>
                                @endif
                                @if (request('jenis_zakat_id'))
                                    @php $jenis = $jenisZakatList->firstWhere('id', request('jenis_zakat_id')); @endphp
                                    <span class="inline-flex items-center px-2 py-0.5 bg-blue-100 text-blue-800 text-xs rounded-full">
                                        {{ $jenis->nama ?? 'Unknown' }}
                                        <button type="button" onclick="removeFilter('jenis_zakat_id')" class="ml-1 text-blue-600 hover:text-blue-800 leading-none">×</button>
                                    </span>
                                @endif
                                @if (request('requires_haul'))
                                    <span class="inline-flex items-center px-2 py-0.5 bg-blue-100 text-blue-800 text-xs rounded-full">
                                        Haul: {{ request('requires_haul') == 'true' ? 'Perlu' : 'Tanpa' }}
                                        <button type="button" onclick="removeFilter('requires_haul')" class="ml-1 text-blue-600 hover:text-blue-800 leading-none">×</button>
                                    </span>
                                @endif
                            </div>
                        </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Informasi Tipe</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nisab</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Persentase</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Haul</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($tipeZakat as $tipe)
                                <tr class="hover:bg-gray-50 transition-colors cursor-pointer expandable-row"
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-medium text-gray-900">{{ $tipe->nama }}</div>
                                        <div class="text-xs text-gray-500 mt-0.5">{{ $tipe->jenisZakat->nama ?? '-' }}</div>
                                    ...
                                    <td class="px-6 py-4">
                                        @if ($tipe->persentase_zakat)
                                            <div class="text-sm font-medium text-gray-900">{{ $tipe->formatted_persentase }}</div>
                                            @if ($tipe->persentase_alternatif)
                                                <div class="text-xs text-gray-500 mt-0.5">Alt: {{ number_format($tipe->persentase_alternatif, 2) }}%</div>
                                            @endif
                                        @else
                                            <span class="text-xs text-gray-400">-</span>
                                        @endif
                                    ...
                                    <td class="px-6 py-4">
                                        @if ($tipe->requires_haul)
                                            <span class="text-sm text-gray-900 whitespace-nowrap">Ya (1 tahun)</span>
                                        @else
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="space-y-2">
                                                            @if ($tipe->nisab_emas_gram)
                                                                <div class="flex justify-between text-sm"><span class="text-gray-600">Emas:</span><span class="font-medium text-gray-900">{{ number_format($tipe->nisab_emas_gram, 2) }} gram</span></div>
                                                            @endif
                                                            @if ($tipe->nisab_perak_gram)
                                                                <div class="flex justify-between text-sm"><span class="text-gray-600">Perak:</span><span class="font-medium text-gray-900">{{ number_format($tipe->nisab_perak_gram, 2) }} gram</span></div>
                                                            @endif
                                                            @if ($tipe->nisab_pertanian_kg)
                                                                <div class="flex justify-between text-sm"><span class="text-gray-600">Pertanian:</span><span class="font-medium text-gray-900">{{ number_format($tipe->nisab_pertanian_kg, 2) }} kg</span></div>
                                                            @endif
                                                            @if ($tipe->nisab_kambing_min)
                                                                <div class="flex justify-between text-sm"><span class="text-gray-600">Kambing:</span><span class="font-medium text-gray-900">min {{ number_format($tipe->nisab_kambing_min) }} ekor</span></div>
                                                            @endif
                                                            @if ($tipe->nisab_sapi_min)
                                                                <div class="flex justify-between text-sm"><span class="text-gray-600">Sapi:</span><span class="font-medium text-gray-900">min {{ number_format($tipe->nisab_sapi_min) }} ekor</span></div>
                                                            @endif
                                                            @if ($tipe->nisab_unta_min)
                                                                <div class="flex justify-between text-sm"><span class="text-gray-600">Unta:</span><span class="font-medium text-gray-900">min {{ number_format($tipe->nisab_unta_min) }} ekor</span></div>
                                                            @endif
                                                        ...
                                                        <div class="space-y-3">
                                                            ...
                                                            <div class="flex items-start">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <p class="text-sm font-medium text-gray-900">{{ $tipe->requires_haul ? 'Memerlukan haul (1 tahun)' : 'Tidak memerlukan haul' }}</p>
                                                                </div>
                                                            </div>
                                                        </div>
                ...
                {{-- ============================================================ --}}
                {{-- MOBILE VIEW — diperbaiki agar fully responsive                --}}
                {{-- ============================================================ --}}
                <div class="md:hidden divide-y divide-gray-100">
                    @foreach ($tipeZakat as $tipe)
                        <div class="expandable-card">
                            {{-- Card Header --}}
                            <div class="px-4 py-3 hover:bg-gray-50 active:bg-gray-100 transition-colors cursor-pointer expandable-row-mobile"
                                data-target="detail-mobile-{{ $tipe->uuid }}">
                                <div class="flex items-start gap-3">
                                    {{-- Left: info --}}
                                    <div class="flex-1 min-w-0">
                                        {{-- Nama zakat — wrap, tidak truncate --}}
                                        <p class="text-sm font-semibold text-gray-900 leading-snug pr-1">
                                            {{ $tipe->nama }}
                                        </p>
                                        {{-- Jenis --}}
                            ...
                            <div id="detail-mobile-{{ $tipe->uuid }}" class="hidden expandable-content-mobile">
                                <div class="bg-gray-50 px-4 py-3 border-t border-gray-100 space-y-4">
                                    ...
                                    {{-- Nisab --}}
                                    @php
                                        $hasNisab = $tipe->nisab_emas_gram || $tipe->nisab_perak_gram || $tipe->nisab_pertanian_kg || $tipe->nisab_kambing_min || $tipe->nisab_sapi_min || $tipe->nisab_unta_min;
                                    @endphp
                                    <div>
                                        <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Detail Nisab</h4>
                                        @if ($hasNisab)
                                    ...
                                    @if ($tipe->persentase_zakat || $tipe->persentase_alternatif || $tipe->keterangan_persentase)
                                        <div>
                                            <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Persentase Zakat</h4>
                                            <div class="bg-white rounded-lg border border-gray-100 divide-y divide-gray-50">
                                                @if ($tipe->persentase_zakat)
                                                    <div class="flex justify-between items-center px-3 py-2 text-sm">
                                                ...
                                                @if ($tipe->keterangan_persentase)
                                                    <div class="px-3 py-2 text-sm">
                                                        <span class="text-gray-600 block mb-0.5">Keterangan</span>
                                                        <p class="text-gray-900">{{ $tipe->keterangan_persentase }}</p>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                                    ...
                                    @if ($tipe->ketentuan_khusus)
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-600 bg-white rounded-lg border border-gray-100 px-3 py-2">
                                                {{ Str::limit($tipe->ketentuan_khusus, 120) }}
                                            </p>
                                        </div>
                                    @endif
                                    ...
                                    <div class="pt-1 flex gap-2">
                                        <a href="{{ route('tipe-zakat.edit', $tipe->uuid) }}"
                                            class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 text-sm font-medium rounded-lg transition-all">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <button type="button"
                                            ...
                                            class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-red-50 border border-red-100 hover:bg-red-100 text-red-700 text-sm font-medium rounded-lg transition-all">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @else
                <div class="p-8 sm:p-12 text-center">
                    ...
                    @if (request('search') || request('jenis_zakat_id') || request('requires_haul'))
                        ...
                        <p class="text-sm text-gray-500 mb-6">Tidak ada data tipe zakat yang cocok dengan filter yang dipilih.</p>
    ...
    <div id="delete-modal"
        ...
        <div class="p-4 sm:p-6 border border-gray-200 w-full max-w-sm shadow-lg rounded-xl sm:rounded-2xl bg-white">
            ...
            <p class="text-xs sm:text-sm text-gray-500 mb-1 text-center">
                Apakah Anda yakin ingin menghapus tipe zakat "<span id="modal-nama" class="font-semibold text-gray-700"></span>"?
            </p>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('click', function(e) {
                ...
                if (toggle) {
                    ...
                    currentDropdownData = { uuid: dropdownUuid, nama: nama };
        ...
        function toggleSearch() {
            ...
            if (searchForm.classList.contains('hidden')) {
                ...
                if (!'{{ request('search') }}') searchInput.value = '';
