{{-- resources/views/amil/transaksi-penerimaan/show-pemantauan.blade.php --}}
{{-- Mendukung semua metode penerimaan: datang_langsung, dijemput, daring --}}
    ...
    <div class="space-y-4 sm:space-y-6">
        ...
        @if (
            $transaksi->metode_penerimaan === 'daring' &&
                in_array($transaksi->metode_pembayaran, ['transfer', 'qris']) &&
        ...
        {{-- [DIJEMPUT] Panel: Menunggu Penjemputan --}}
        @if ($transaksi->metode_penerimaan === 'dijemput' && $transaksi->status_penjemputan === 'menunggu')
            <div class="bg-blue-50 border border-blue-200 rounded-xl sm:rounded-2xl overflow-hidden">
                <div class="px-4 sm:px-6 py-3 border-b border-blue-200 bg-blue-100/60 flex items-center gap-2">
                    ...
                    <h3 class="text-sm font-semibold text-blue-800">Menunggu Penjemputan</h3>
                </div>
                <div class="p-4 sm:p-6">
                    <p class="text-sm text-blue-800">
                        Request penjemputan telah diterima. Amil
                        <strong>{{ optional(optional($transaksi->amil)->pengguna)->name ?? 'yang ditugaskan' }}</strong>
        ...
        {{-- [DIJEMPUT] Panel: Dalam Proses Penjemputan --}}
        @if (
            ...
            <div class="bg-indigo-50 border border-indigo-200 rounded-xl sm:rounded-2xl overflow-hidden">
                <div class="px-4 sm:px-6 py-3 border-b border-indigo-200 bg-indigo-100/60 flex items-center gap-2">
                    ...
                    <h3 class="text-sm font-semibold text-indigo-800">
                        ...
                        @php
                            $labelPenjemputan = [
                                'diterima' => 'Permintaan Diterima Amil',
                                'dalam_perjalanan' => 'Amil Dalam Perjalanan',
                                'sampai_lokasi' => 'Amil Telah Sampai Lokasi',
                            ];
                        @endphp
                        {{ $labelPenjemputan[$transaksi->status_penjemputan] ?? $transaksi->status_penjemputan }}
                    </h3>
                </div>
                <div class="p-4 sm:p-6">
                    <p class="text-sm text-indigo-800">
                        Amil <strong>{{ optional(optional($transaksi->amil)->pengguna)->name ?? '-' }}</strong>
                        sedang dalam proses menjemput zakat Anda.
                    </p>
                </div>
            </div>
        @endif

        {{-- Panel: Sudah Dikonfirmasi (Transfer/QRIS) --}}
                ...
                <div>
                    ...
                    @if ($transaksi->dikonfirmasi_oleh)
                        <p class="text-xs text-green-700 mt-0.5">
                            Oleh:
                            <strong>{{ optional($transaksi->dikonfirmasiOleh)->username ?? $transaksi->dikonfirmasi_oleh }}</strong>
        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-card border border-gray-100 overflow-hidden animate-slide-up">
            ...
            <div class="p-4 sm:p-6 space-y-6 sm:space-y-8">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        @if (in_array($transaksi->metode_pembayaran, ['transfer', 'qris']))
                            @php
                                ...
                                $konfBadge = $konfBadgeMap[$transaksi->konfirmasi_status] ?? null;
                            @endphp
                            @if ($konfBadge)
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border {{ $konfBadge[0] }}">
                                    {{ $konfBadge[1] }}
                                </span>
                            @endif
                        @endif

                        {{-- Badge Status Penjemputan (khusus dijemput) --}}
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-start text-sm text-gray-900 gap-2">
                            ...
                            <div>
                                @php
                                    $metodeFisik = in_array($transaksi->metode_pembayaran, [
                                        'beras',
                                        'bahan_mentah',
                                        'makanan_matang',
                                    ]);
                                ...
                                @if ($metodeFisik)
                                    <p class="font-semibold text-amber-600">
                                        ...
                                        @elseif ($transaksi->metode_pembayaran === 'bahan_mentah')
                                            Bahan Mentah
                                            @if ($transaksi->fidyah_total_berat_kg)
                                                ({{ $transaksi->fidyah_total_berat_kg }} kg)
                                            @endif
                                        @elseif ($transaksi->metode_pembayaran === 'makanan_matang')
                                            Makanan Siap Santap
                                            @if ($transaksi->fidyah_jumlah_box)
                                                ({{ $transaksi->fidyah_jumlah_box }} box)
                                            @endif
                                        @endif
                                    </p>
                                @else
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            ...
                            {{-- Lokasi Penjemputan (khusus dijemput) --}}
                ...
                {{-- ============================================================
                     DETAIL FIDYAH — Tampil jika transaksi adalah fidyah
                ============================================================ --}}
                    ...
                    <div>
                        ...
                        <div class="bg-orange-50 border border-orange-200 rounded-xl p-4 sm:p-5">
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                <div>
                                    <label
                                        class="block text-xs font-medium text-orange-700 uppercase tracking-wider mb-1">Tipe
                                        Fidyah</label>
                                    <p class="text-sm font-medium text-gray-900">
                                        ...
                                        @endphp
                                        {{ $labelFidyah[$transaksi->fidyah_tipe] ?? ucfirst($transaksi->fidyah_tipe) }}
                                    </p>
                                </div>
                                <div>
                                    <label
                                        class="block text-xs font-medium text-orange-700 uppercase tracking-wider mb-1">Jumlah
                                        Hari</label>
                                    <p class="text-sm font-medium text-gray-900">{{ $transaksi->fidyah_jumlah_hari }} hari
                                    </p>
                                </div>

                                {{-- Detail Fidyah Mentah --}}
                                @if ($transaksi->fidyah_tipe === 'mentah')
                                    <div>
                                        <label
                                            class="block text-xs font-medium text-orange-700 uppercase tracking-wider mb-1">Bahan
                                            Pokok</label>
                                        <p class="text-sm text-gray-900">{{ $transaksi->fidyah_nama_bahan ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <label
                                            class="block text-xs font-medium text-orange-700 uppercase tracking-wider mb-1">Berat
                                            per Hari</label>
                                        <p class="text-sm text-gray-900">
                                            {{ $transaksi->fidyah_berat_per_hari_gram ?? 675 }} gram</p>
                                    </div>
                                    @if ($transaksi->fidyah_total_berat_kg)
                                        <div>
                                            <label
                                                class="block text-xs font-medium text-orange-700 uppercase tracking-wider mb-1">Total
                                                Berat</label>
                                            <p class="text-sm font-semibold text-gray-900">
                                                {{ $transaksi->fidyah_total_berat_kg }} kg</p>
                                        </div>
                                    @endif
                                @endif

                                {{-- Detail Fidyah Matang --}}
                                @if ($transaksi->fidyah_tipe === 'matang')
                                    @if ($transaksi->fidyah_jumlah_box)
                                        <div>
                                            <label
                                                class="block text-xs font-medium text-orange-700 uppercase tracking-wider mb-1">Jumlah
                                                Box</label>
                                            <p class="text-sm font-semibold text-gray-900">
                                                {{ $transaksi->fidyah_jumlah_box }} box</p>
                                        </div>
                                    @endif
                                    @if ($transaksi->fidyah_menu_makanan)
                                        <div>
                                            <label
                                                class="block text-xs font-medium text-orange-700 uppercase tracking-wider mb-1">Menu</label>
                                    ...
                                    @if ($transaksi->fidyah_harga_per_box)
                                        <div>
                                            <label
                                                class="block text-xs font-medium text-orange-700 uppercase tracking-wider mb-1">Harga
                                                per Box</label>
                                            <p class="text-sm text-gray-900">Rp
                                                {{ number_format($transaksi->fidyah_harga_per_box, 0, ',', '.') }}</p>
                                        </div>
                                    @endif
                                    @if ($transaksi->fidyah_cara_serah)
                                        <div>
                                            <label
                                                class="block text-xs font-medium text-orange-700 uppercase tracking-wider mb-1">Cara
                                                Serah</label>
                                            <p class="text-sm text-gray-900">
                                                @php
                                                    $labelSerah = [
                                                        'dibagikan' => 'Dibagikan ke Penerima',
                                                        'dijamu' => 'Dijamu Langsung',
                                                        'via_lembaga' => 'Via Lembaga',
                                                    ];
                                                @endphp
                                                {{ $labelSerah[$transaksi->fidyah_cara_serah] ?? $transaksi->fidyah_cara_serah }}
                                            </p>
                                        </div>
                                    @endif
                                @endif
                                ...
                                @if ($transaksi->fidyah_tipe === 'tunai')
                                    <div>
                                        <label
                                            class="block text-xs font-medium text-orange-700 uppercase tracking-wider mb-1">Total
                                            Dibayar</label>
                                        <p class="text-sm font-semibold text-green-600">{{ $transaksi->jumlah_formatted }}
                                        </p>
                                    </div>
                                @endif
                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            ...
                            @if ($transaksi->jumlah_infaq > 0)
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Infaq
                                        Sukarela</label>
                            ...
                            {{-- Nama Jiwa (jika ada) --}}
                            @if ($transaksi->nama_jiwa_json && count($transaksi->nama_jiwa_json) > 0)
                                <div>
                        ...
                        <div class="space-y-4">
                            ...
                            {{-- Jadwal Penjemputan (khusus dijemput) --}}
                            @if ($transaksi->metode_penerimaan === 'dijemput' && $transaksi->tanggal_penjemputan)
                                <div>
                                    <label
                                        class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Jadwal
                                        Penjemputan</label>
                                    <p class="text-sm text-gray-900">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                            ...
                            <div class="flex items-center gap-3 mb-3">
                                @if ($transaksi->amil->foto_url && file_exists(public_path('storage/' . $transaksi->amil->foto_url)))
                                    <div
                                        ...
                                        <img src="{{ asset('storage/' . $transaksi->amil->foto_url) }}" alt="Foto Amil"
                                            class="w-full h-full object-cover">
                                    </div>
                                @else
                                    <div
                                        class="w-12 h-12 rounded-full {{ $bgColor }} flex items-center justify-center text-white font-bold text-lg shadow-md flex-shrink-0">
                                        {{ $inisialAmil }}
                                    </div>
                                @endif
                                <div>
                        ...
                        {{-- Status Penjemputan Timeline (khusus dijemput) --}}
                        @if ($transaksi->metode_penerimaan === 'dijemput' && $transaksi->status_penjemputan)
                            <div class="mt-5">
                                ...
                                <div class="space-y-2 mt-3">
                                    @foreach ([['waktu_request', 'Request Dibuat', 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z'], ['waktu_diterima_amil', 'Diterima Amil', 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'], ['waktu_berangkat', 'Amil Berangkat', 'M13 7l5 5m0 0l-5 5m5-5H6'], ['waktu_sampai', 'Sampai Lokasi', 'M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z'], ['waktu_selesai', 'Penjemputan Selesai', 'M5 13l4 4L19 7']] as [$field, $label, $path])
                                        @if ($transaksi->$field)
                                            <div class="flex items-center gap-2 text-xs">
                                                ...
                                                <span
                                                    class="text-gray-800 font-medium">{{ $transaksi->$field->format('d/m/Y H:i') }}</span>
                                            </div>
                                        @endif
                                    @endforeach
                                </div>
                            ...
                            @if (in_array($transaksi->metode_pembayaran, ['transfer', 'qris']))
                                <div class="pt-3 border-t border-gray-200">
                                    ...
                                    @php
                                        ...
                                        [$kClass, $kLabel] = $konfStatusMap[$transaksi->konfirmasi_status] ?? [
                                            'bg-gray-100 text-gray-600',
                                            '—',
                                        ];
                                    @endphp
                                    <span
                                        class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium {{ $kClass }}">
                                        {{ $kLabel }}
                                    </span>
                                    @if ($transaksi->dikonfirmasi_oleh && $transaksi->konfirmasi_at)
