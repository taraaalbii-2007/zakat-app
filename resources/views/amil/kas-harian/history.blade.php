    ...
    <div class="bg-white rounded-xl sm:rounded-2xl shadow-card border border-gray-100 overflow-hidden animate-slide-up">
        ...
        <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-base sm:text-lg font-semibold text-gray-900">Riwayat Kas Harian</h2>
                    <p class="text-xs sm:text-sm text-gray-500 mt-1">Laporan kas harian per tanggal</p>
                </div>
                <div class="flex items-center gap-2">
                    <a href="{{ route('kas-harian.index') }}"
                       class="inline-flex items-center justify-center px-3 py-2 border border-gray-300 text-xs font-medium rounded-lg text-gray-600 bg-white hover:bg-gray-50 transition-colors">
                    ...
                    <a href="{{ route('kas-harian.export-excel', request()->all()) }}"
                       class="inline-flex items-center px-3 py-2 bg-green-600 hover:bg-green-700 text-white text-xs font-medium rounded-lg transition-all shadow-sm">
                        <svg class="w-3.5 h-3.5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-4 sm:p-6">
            ...
            <div class="pb-6 border-b border-gray-200">
                <div class="flex flex-col sm:flex-row items-start gap-4">
                    ...
                    <div class="w-full">
                        <h3 class="text-xl sm:text-2xl font-bold text-gray-900">Riwayat Kas Harian</h3>
                        <p class="text-sm text-gray-500 mt-1">Rekap seluruh kas harian masjid</p>
            ...
            <div class="mt-6">
                ...
                {{-- TAB: Ringkasan                                          --}}
                {{-- ═══════════════════════════════════════════════════════ --}}
                <div id="content-ringkasan" class="tab-content mt-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">

                        {{-- Statistik --}}
                        <div class="space-y-4">
                            <h4 class="text-sm font-semibold text-gray-900 uppercase tracking-wider">Statistik Periode</h4>
                            <div class="space-y-3">
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <p class="text-sm font-medium text-gray-900">{{ $kasHarian->total() }} hari</p>
                                    </div>
                                </div>
                                <div class="flex items-start">
                        ...
                        {{-- Progress & Status --}}
                        <div class="space-y-4">
                            <h4 class="text-sm font-semibold text-gray-900 uppercase tracking-wider">Komposisi Kas</h4>
                            <div class="space-y-4">
                                @php
                                ...
                                <div class="p-4 bg-gradient-to-br from-green-50 to-green-100 rounded-lg border border-green-200">
                                    ...
                                    <div class="flex justify-between text-xs text-gray-600">
                                        <span class="font-medium">Rp {{ number_format($totalPenerimaan, 0, ',', '.') }}</span>
                                ...
                                <div class="p-4 bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg border border-orange-200">
                                    ...
                                    <div class="flex justify-between text-xs text-gray-600">
                                        <span class="font-medium">Rp {{ number_format($totalPenyaluran, 0, ',', '.') }}</span>
                                        <span>dari total arus</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <h4 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Status Kas</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div class="p-4 bg-green-50 border border-green-200 rounded-xl">
                                <p class="text-xs text-green-600 font-medium mb-1">Kas Ditutup (Closed)</p>
                                <p class="text-2xl font-bold text-green-700">{{ $ringkasan['total_closed'] ?? 0 }}</p>
                                <p class="text-xs text-green-500 mt-1">hari</p>
                            </div>
                            <div class="p-4 bg-blue-50 border border-blue-200 rounded-xl">
                                <p class="text-xs text-blue-600 font-medium mb-1">Kas Terbuka (Open)</p>
                                <p class="text-2xl font-bold text-blue-700">{{ $ringkasan['total_open'] ?? 0 }}</p>
                            ...
                            <div class="p-4 bg-purple-50 border border-purple-200 rounded-xl">
                                ...
                                <p class="text-2xl font-bold text-purple-700">{{ $ringkasan['total_transaksi'] ?? 0 }}</p>
                ...
                {{-- TAB: Daftar Kas                                         --}}
                {{-- ═══════════════════════════════════════════════════════ --}}
                <div id="content-daftar" class="tab-content hidden mt-6">
                            ...
                            <div class="bg-white border border-gray-200 rounded-xl p-4">
                                <div class="flex items-start justify-between mb-3">
                                    ...
                                    <div class="flex items-center gap-2">
                                        {!! $kas->status_badge !!}
                                        <a href="{{ route('kas-harian.index', ['tanggal' => $kas->tanggal->format('Y-m-d')]) }}"
                                           class="text-xs text-primary hover:underline font-medium">Detail →</a>
                                    </div>
                                </div>
                    ...
                    @else
                        <div class="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                            ...
                            <h3 class="mt-3 text-sm font-medium text-gray-700">Belum Ada Riwayat</h3>
                    ...
                    <div class="w-full">
                        ...
                        <form method="GET" action="{{ route('kas-harian.history') }}" id="filter-form">
                            ...
                            <div class="flex items-center justify-center gap-3">
                                ...
                                @if(request('start_date') || request('end_date') || request('status'))
                                    <a href="{{ route('kas-harian.history') }}"
                                       class="inline-flex items-center px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm font-medium rounded-lg transition-all">
                                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div>
                                    ...
                                    <div class="text-xs text-yellow-700 space-y-0.5">
                                        @if(request('start_date'))
                                            <p>Dari: <span class="font-medium">{{ \Carbon\Carbon::parse(request('start_date'))->translatedFormat('d F Y') }}</span></p>
                                        @endif
                                        @if(request('end_date'))
                                            <p>Sampai: <span class="font-medium">{{ \Carbon\Carbon::parse(request('end_date'))->translatedFormat('d F Y') }}</span></p>
                                        @endif
                                        @if(request('status'))
                                            <p>Status: <span class="font-medium capitalize">{{ request('status') }}</span></p>
                                        @endif
                                    </div>
                                </div>
                            </div>
