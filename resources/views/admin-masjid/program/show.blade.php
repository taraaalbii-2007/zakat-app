...
@section('content')
            ...
            <div class="p-4 sm:p-6">
                ...
                <div class="pb-6 border-b border-gray-200">
                    <div class="flex flex-col sm:flex-row items-start gap-4">
                        @if (!empty($program->foto_poster))
                            <div class="flex-shrink-0">
                                <img src="{{ Storage::url($program->foto_poster) }}" alt="Poster Program"
                ...
                <div class="mt-6">
                    ...
                    {{-- Tab Content: Informasi Program --}}
                    <div id="content-info" class="tab-content mt-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {{-- Informasi Dasar --}}
                            <div class="space-y-4">
                                <h4 class="text-sm font-semibold text-gray-900 uppercase tracking-wider">Informasi Dasar
                                </h4>
                            ...
                            {{-- Progress --}}
                            <div class="space-y-4">
                                <h4 class="text-sm font-semibold text-gray-900 uppercase tracking-wider">Progress Pencapaian
                                </h4>

                                <div class="space-y-4">
                                    @if ($program->target_dana)
                                        <div
                                            class="p-4 bg-gradient-to-br from-primary/5 to-primary/10 rounded-lg border border-primary/20">
                                            <div class="flex justify-between items-center mb-2">
                                                <span class="text-xs font-medium text-gray-700">Pencapaian Dana</span>
                                                <span
                                                    class="text-sm font-bold text-primary">{{ $program->progress_dana }}%</span>
                                            </div>
                                            ...
                                            <div class="flex justify-between text-xs text-gray-600">
                                                <span class="font-medium">Rp
                                                    {{ number_format($program->realisasi_dana ?? 0, 0, ',', '.') }}</span>
                                    ...
                                    @if ($program->target_mustahik)
                                        <div
                                            class="p-4 bg-gradient-to-br from-green-50 to-green-100 rounded-lg border border-green-200">
                                            <div class="flex justify-between items-center mb-2">
                                                <span class="text-xs font-medium text-gray-700">Pencapaian Mustahik</span>
                                                <span
                                                    class="text-sm font-bold text-green-600">{{ $program->progress_mustahik ?? 0 }}%</span>
                                            </div>
                                            <div class="w-full bg-gray-200 rounded-full h-3 mb-2">
                                                <div class="bg-green-500 h-3 rounded-full transition-all duration-500"
                                                    style="width: {{ $program->progress_mustahik ?? 0 }}%"></div>
                                            </div>
                                            <div class="flex justify-between text-xs text-gray-600">
                                                <span class="font-medium">{{ $program->realisasi_mustahik ?? 0 }}
                                                    orang</span>
                                    ...
                                    @else
                                        <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                                            <div class="flex items-center justify-between">
                                                ...
                                                <span
                                                    class="text-sm font-bold text-green-600">{{ $program->realisasi_mustahik ?? 0 }}
                                                    orang</span>
                                            </div>
                                        </div>
                                    @endif
                        ...
                        <div class="mt-6 pt-6 border-t border-gray-200">
                            <h4 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Deskripsi Program
                            </h4>
                        ...
                        <div class="mt-6 pt-6 border-t border-gray-200">
                            <div class="flex flex-wrap gap-4 text-xs text-gray-500">
                                <div>
                                    <span>Dibuat:</span>
                                    <span
                                        class="font-medium text-gray-700">{{ $program->created_at->format('d M Y, H:i') }}</span>
                                </div>
                                <div>
                                    <span>Diperbarui:</span>
                                    <span
                                        class="font-medium text-gray-700">{{ $program->updated_at->format('d M Y, H:i') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>

                    {{-- Tab Content: Penerimaan Dana --}}
                    <div id="content-penerimaan" class="tab-content hidden mt-6">
                        <div class="text-center py-12">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900">Belum ada data penerimaan</h3>
                            <p class="mt-1 text-sm text-gray-500">Data penerimaan dana untuk program ini akan muncul di
                                sini</p>
                            @if ($program->status === 'aktif')
                                <div class="mt-6">
                                    <button type="button" onclick="alert('Fitur penerimaan dana akan segera hadir')"
                    ...
                    <div id="content-penyaluran" class="tab-content hidden mt-6">
                        <div class="text-center py-12">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900">Belum ada data penyaluran</h3>
                            <p class="mt-1 text-sm text-gray-500">Data penyaluran zakat untuk program ini akan muncul di
                                sini</p>
                            @if ($program->status === 'aktif' && ($program->realisasi_dana ?? 0) > 0)
                                <div class="mt-6">
                    ...
                    <div id="content-gallery" class="tab-content hidden mt-6">
                        ...
                        @else
                            <div class="text-center py-12">
                                ...
                                <h3 class="mt-2 text-sm font-medium text-gray-900">Belum ada foto kegiatan</h3>
                                <p class="mt-1 text-sm text-gray-500">Upload foto dokumentasi kegiatan program zakat</p>
                                ...
                                @if (!in_array($program->status, ['selesai', 'dibatalkan']))
                                    <div class="mt-6">
                                        <form action="{{ route('program-zakat.upload-foto', $program->uuid) }}"
                                            ...
                                            @csrf
                                            <input type="file" name="foto" id="foto-upload-first"
                                                accept="image/jpeg,image/png,image/jpg"
                                                class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-primary file:text-white hover:file:bg-primary-600 transition-colors">
                                            <button type="submit"
                                            ...
                                            <p class="text-xs text-gray-500 text-center">Format: JPEG, PNG, JPG. Maksimal
                                                2MB</p>
                                        </form>
                                    </div>
                                @endif
            ...
            <div class="px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 border-t border-gray-200">
                <div class="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        {{-- Button Hapus hanya untuk status draft --}}
                        @if ($program->status === 'draft')
    ...
    <div id="status-modal"
        class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
        <div class="p-4 sm:p-6 border border-gray-200 w-full max-w-sm shadow-lg rounded-xl sm:rounded-2xl bg-white">
            <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4 text-center" id="status-modal-title">
            </h3>
...
@push('scripts')
    <script>
        ...
        function showStatusModal(status) {
            ...
            if (status === 'aktif') {
                statusTitle = 'Aktifkan Program';
                statusMessage = 'Program akan diaktifkan dan dapat menerima donasi serta penyaluran.';
            } else if (status === 'selesai') {
                statusTitle = 'Tandai Selesai';
                statusMessage = 'Program akan ditandai sebagai selesai dan tidak dapat menerima donasi baru.';
            } else if (status === 'dibatalkan') {
                statusTitle = 'Batalkan Program';
                statusMessage = 'Program akan dibatalkan dan tidak dapat diaktifkan kembali.';
            }
        ...
        function deletePhoto(index) {
            if (confirm('Apakah Anda yakin ingin menghapus foto ini?')) {
                const url = `/program-zakat/{{ $program->uuid }}/foto/${index}`;
