<aside id="sidebar"
    class="fixed inset-y-0 left-0 z-50 w-64 bg-white text-gray-800 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-out flex flex-col shadow-xl"
...
/* Section label - jarak lebih rapat */
.sb-section-label {
    font-size: 10px;
    font-weight: 700;
...
/* Active indicator - lebih refined */
.sb-item::before {
    content: '';
    position: absolute;
    left: 0;
...
.sb-item:hover .sb-icon { color: #475569; transform: scale(1.02); }
...
.sb-sub-item:hover svg { color: #64748b; }
.sb-sub-item.active svg { color: #16a34a; }

/* Micro label - lebih subtle */
.sb-micro-label {
    font-size: 10px;
    font-weight: 600;
...
/* Logo area - jarak lebih rapat ke menu */
.logo-container {
    padding: 12px 16px 12px;  /* padding top dari 20px jadi 12px */
    border-bottom: 1px solid #f1f5f9;
}

/* Navigation - kurangi padding top */
#sidebar nav {
    padding-top: 4px;
}

/* Footer - lebih elegant */
.sidebar-footer {
    padding: 14px 16px;
        ...
        <div class="flex items-center gap-3">
            @if (!empty($appConfig->logo_aplikasi))
                <img src="{{ asset('storage/' . $appConfig->logo_aplikasi) }}"
                    ...
                    class="w-10 h-10 rounded-xl object-cover flex-shrink-0 ring-2 ring-green-100 ring-offset-0">
            @else
            ...
            <div class="min-w-0 flex-1">
                <h1 class="text-sm font-extrabold text-slate-800 leading-tight truncate">
                    {{ $appConfig->nama_aplikasi ?? 'Aplikasi Zakat' }}
                </h1>
                <p class="text-[10px] text-slate-400 mt-0.5 truncate font-medium">
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4"
        ...
        @if ($isSuperadmin)
            ...
            <a href="{{ route('dashboard') }}" class="sb-item {{ $isActive('dashboard') ? 'active' : '' }}">
                <span class="sb-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg></span>
                <span>Dashboard</span>
            </a>
            ...
            <details class="sb-group" {{ $isMasterOpen ? 'open' : '' }}>
                <summary class="sb-item {{ $isMasterOpen ? 'active' : '' }}">
                    <span class="sb-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"/></svg></span>
                ...
                <div class="sb-sub-list">
                    ...
                    <a href="{{ route('tipe-zakat.index') }}" class="sb-sub-item {{ $isActive('tipe-zakat') ? 'active' : '' }}">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h10M7 11h10M7 15h6"/></svg>
                    ...
                    <a href="{{ route('harga-emas-perak.index') }}" class="sb-sub-item {{ $isActive('harga-emas-perak') ? 'active' : '' }}">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.657 0 3 .895 3 2s-1.343 2-3 2m0-4v2m0 10v2"/></svg>
            ...
            <a href="{{ route('pengguna.index') }}" class="sb-item {{ $isActive('pengguna') ? 'active' : '' }}">
                <span class="sb-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg></span>
                <span>Pengguna</span>
            </a>
            ...
            <details class="sb-group" {{ $isArtikelOpen ? 'open' : '' }}>
                <summary class="sb-item {{ $isArtikelOpen ? 'active' : '' }}">
                    <span class="sb-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z"/></svg></span>
                    <span>Bulletin</span>
                ...
                <div class="sb-sub-list">
                    <a href="{{ route('superadmin.kategori-bulletin.index') }}" class="sb-sub-item {{ $isActive('kategori-bulletin') ? 'active' : '' }}">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l5 5a2 2 0 01.586 1.414V19a2 2 0 01-2 2H7a2 2 0 01-2-2V5a2 2 0 012-2z"/></svg>
            ...
            <a href="{{ route('lembaga.index') }}" class="sb-item {{ $isActive('lembaga') ? 'active' : '' }}">
                <span class="sb-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg></span>
                <span>Lembaga</span>
            </a>
            ...
            <details class="sb-group" {{ $isTrxOpen ? 'open' : '' }}>
                <summary class="sb-item {{ $isTrxOpen ? 'active' : '' }}">
                    <span class="sb-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg></span>
                    <span>Transaksi</span>
                    <svg class="sb-chevron" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"/></svg>
                </summary>
                <div class="sb-sub-list">
            ...
            <a href="{{ route('superadmin.testimoni.index') }}" class="sb-item {{ $isActive('superadmin.testimoni') ? 'active' : '' }}">
                <span class="sb-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg></span>
                <span>Kelola Testimoni</span>
            </a>
            <a href="{{ route('laporan-konsolidasi.index') }}" class="sb-item {{ $isActive('laporan-konsolidasi') ? 'active' : '' }}">
                <span class="sb-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg></span>
                <span>Keuangan Seluruh Lembaga</span>
            </a>
            <a href="{{ route('log-aktivitas.index') }}" class="sb-item {{ $isActive('log-aktivitas') ? 'active' : '' }}">
                <span class="sb-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></span>
        ...
        @if ($isAdminLembaga)
            ...
            <a href="{{ route('transaksi-penyaluran.index') }}" class="sb-item {{ $isActive('transaksi-penyaluran') ? 'active' : '' }}">
                <span class="sb-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg></span>
            ...
            <a href="{{ route('admin-lembaga.setor-kas.pending') }}" class="sb-item {{ $isActive('setor-kas') ? 'active' : '' }}">
                <span class="sb-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg></span>
        ...
        @if ($isMuzakki)
            ...
            <a href="{{ route('riwayat-transaksi-muzakki.index') }}" class="sb-item {{ $isActive('riwayat-transaksi-muzakki') ? 'active' : '' }}">
                <span class="sb-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg></span>
...
<button id="sidebar-toggle"
    class="lg:hidden fixed top-5 left-5 z-50 p-2.5 rounded-xl bg-white text-gray-500 shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105 active:scale-95"
...
<div id="sidebar-overlay" class="lg:hidden fixed inset-0 bg-black/30 z-40 hidden backdrop-blur-sm transition-all duration-300"></div>
        ...
        window.addEventListener('resize', () => {
            ...
            resizeTimer = setTimeout(() => {
                if (window.innerWidth >= 1024) {
                    ...
                    // Keep open state on mobile resize
                }
            }, 150);
        });
    })();
