        ...
        <div class="flex items-center space-x-3.5">
            ...
            @else
                <div class="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0"
                    style="background: linear-gradient(135deg, #17a34a 0%, #22c55e 100%);">
                        ...
                        <ul class="{{ $subBorder }}">
                            ...
                            <li>
                                <a href="{{ route('superadmin.bulletin.index') }}"
                                    ...
                                    <span class="flex-1">Kelola Bulletin</span>
                                    @if (!empty($bulletinPendingCount) && $bulletinPendingCount > 0)
                                        <span
                                            class="inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 text-[10px] font-bold rounded-full bg-amber-400 text-white leading-none">
                                            {{ $bulletinPendingCount > 9 ? '9' : $bulletinPendingCount }}
                                        </span>
                                    @endif
                                </a>
                            </li>
            ...
            @if ($isAdminLembaga)
                ...
                <li>
                    <a href="{{ route('mustahik.index') }}"
                        class="flex items-center px-3 py-2.5 rounded-xl transition-all duration-150 text-[13px] {{ str_contains($currentRoute, 'mustahik') ? $activeClass : $inactiveClass }}"
                        @if (str_contains($currentRoute, 'mustahik')) style="background: #f0fdf4;" @endif>
                        <svg class="w-4 h-4 flex-shrink-0 mr-3 {{ str_contains($currentRoute, 'mustahik') ? 'text-[#17a34a]' : 'text-gray-500' }}"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
                        </svg>
                        <span class="flex-1">Data Mustahik</span>
                        @if (!empty($pendingMustahikCount) && $pendingMustahikCount > 0)
                            <span
                                ...
                                {{ $pendingMustahikCount > 99 ? '99+' : $pendingMustahikCount }}
                            </span>
                        @endif
                    </a>
                </li>
                ...
                <li>
                    <a href="{{ route('transaksi-penyaluran.index') }}"
                        class="flex items-center px-3 py-2.5 rounded-xl transition-all duration-150 text-[13px] {{ str_contains($currentRoute, 'transaksi-penyaluran') ? $activeClass : $inactiveClass }}"
                        @if (str_contains($currentRoute, 'transaksi-penyaluran')) style="background: #f0fdf4;" @endif>
                        <svg class="w-4 h-4 flex-shrink-0 mr-3 {{ str_contains($currentRoute, 'transaksi-penyaluran') ? 'text-[#17a34a]' : 'text-gray-500' }}"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                        </svg>
                ...
                <li>
                    <a href="{{ route('admin-lembaga.setor-kas.pending') }}"
                        class="flex items-center px-3 py-2.5 rounded-xl transition-all duration-150 text-[13px] {{ str_contains($currentRoute, 'setor-kas') ? $activeClass : $inactiveClass }}"
                        @if (str_contains($currentRoute, 'setor-kas')) style="background: #f0fdf4;" @endif>
                        <svg class="w-4 h-4 flex-shrink-0 mr-3 {{ str_contains($currentRoute, 'setor-kas') ? 'text-[#17a34a]' : 'text-gray-500' }}"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z" />
                        </svg>
                        <span class="flex-1">Setor Kas Amil</span>
                        @if (!empty($pendingSetorKasCount) && $pendingSetorKasCount > 0)
                            <span
                                class="inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 text-[10px] font-bold rounded-full bg-red-500 text-white leading-none">
                                {{ $pendingSetorKasCount > 99 ? '99+' : $pendingSetorKasCount }}
                            </span>
                        @endif
                    </a>
                </li>
