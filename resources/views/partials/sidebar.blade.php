<aside id="sidebar"
    class="fixed inset-y-0 left-0 z-50 w-64 bg-[#2d6a2d] text-white transform -translate-x-full lg:translate-x-0 transition-transform duration-300 shadow-xl flex flex-col">
    <!-- Logo & Brand -->
    ...
    <nav class="flex-1 px-2 py-3 overflow-y-auto">
        <ul class="space-y-0.5">
            @php
                ...
                $summaryClass =
                    'flex items-center w-full px-2 py-2 rounded-md hover:bg-white/10 hover:text-white cursor-pointer text-sm text-white/80 list-none';
                ...
                <li>
                    <details class="group" {{ $isSuperadminMasterOpen ? 'open' : '' }}>
                        <summary class="{{ $summaryClass }} text-[13px]">
                            ...
                            <svg class="w-3.5 h-3.5 flex-shrink-0 transform group-open:rotate-180 transition-transform"
            ...
            @if ($isAdminMasjid)
                ...
                <li>
                    <a href="{{ route('admin-masjid.muzaki.index') }}"
                        class="flex items-center space-x-2.5 px-2 py-2 rounded-md transition-colors text-[13px] {{ str_contains($currentRoute, 'admin-masjid.muzaki') ? $activeClass : $inactiveClass }}">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                        ...
                        <ul class="{{ $subBorder }}">
                            ...
                            <li>
                                <a href="{{ route('amil.setor-kas.index') }}"
                                    class="flex items-center space-x-2 px-2 py-1.5 text-xs rounded transition-colors {{ str_contains($currentRoute, 'setor-kas') ? $subActive : $subInactive }}">
                                    <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z" />
    ...
    <div class="px-4 py-3 border-t border-white/10">
        <div class="text-center">
            <p class="text-xs text-white/40">© {{ date('Y') }} {{ $appConfig->nama_aplikasi ?? 'Niat Zakat' }}
            </p>
...
<button id="sidebar-toggle"
    class="lg:hidden fixed top-4 left-4 z-50 p-2 rounded-lg bg-[#2d6a2d] text-white shadow-lg">
