...
@php
    ...
    /**
     * MUZAKKI — Notifikasi status transaksi
     */
...
<div id="logout-modal"
    ...
    <div id="logout-card"
        ...
        <div class="p-7">
            ...
            <h2 class="text-[17px] font-bold text-gray-900 text-center mb-1.5" style="letter-spacing: -0.02em;">
                Keluar dari Akun?
            </h2>
            ...
            <div class="flex items-center space-x-3 px-4 py-3 rounded-xl mb-6"
                ...
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold flex-shrink-0 {{ $badgeClass }}">
                    {{ $roleLabel }}
                </span>
            </div>
            ...
            <div class="flex items-center space-x-3">
                <button onclick="closeLogoutModal()"
                        class="flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold text-gray-700 transition-all duration-200
                ...
                <form method="POST" action="{{ route('logout') }}" id="logout-form" class="flex-1">
                    @csrf
                    <button type="submit"
                            class="w-full flex items-center justify-center space-x-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white transition-all duration-200
                                   active:scale-95"
...
<header class="sticky top-0 z-40"
        style="background: #ffffff; border-bottom: 1px solid #eef0f2;">
                ...
                <div class="relative">
                    <button onclick="toggleNotifications()"
                            ...
                            class="relative flex items-center justify-center w-9 h-9 rounded-xl transition-all duration-200
                                   text-gray-500 hover:text-[#1f5c1f] hover:bg-[#f0f7f0]"
                            title="Notifikasi">
                                ...
                                <a href="{{ $notif['url'] }}"
                                   ...
                                   style="border-bottom: 1px solid #f9fafb;">
                                    ...
                                    <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5
                                        {{ $notif['color'] === 'emerald' ? 'bg-emerald-50' :
                                           ($notif['color'] === 'amber'   ? 'bg-amber-50'  :
                                           ($notif['color'] === 'red'     ? 'bg-red-50'    :
                                           ($notif['color'] === 'blue'    ? 'bg-blue-50'   : 'bg-gray-50'))) }}">
                    ...
                    <div id="user-menu-dropdown"
                            ...
                            <div class="flex items-center space-x-2.5">
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center gap-1.5 flex-wrap">
                                        ...
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded-md text-[9.5px] font-semibold {{ $badgeClass }}">
                                            {{ $roleLabel }}
                                        </span>
                                    </div>
                        ...
                        <div class="py-1">
                            ...
                            @if($isSuperadmin)
                                <a href="{{ route('superadmin.profil.show') }}"
                                   class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors
                                          {{ request()->routeIs('superadmin.profil.*') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                ...
                                <a href="{{ route('konfigurasi-global.show') }}"
                                   class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors
                                          {{ request()->routeIs('konfigurasi-global.*') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                                         style="background: #f3f4f6;">
                                        <svg class="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            @if($isAdminMasjid)
                                <a href="{{ route('admin-masjid.profil.show') }}"
                                   class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors
                                          {{ request()->routeIs('admin-masjid.profil.*') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                ...
                                <a href="{{ route('konfigurasi-integrasi.show') }}"
                                   class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors
                                          {{ request()->routeIs('konfigurasi-integrasi.*') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style="background: #f3f4f6;">
                                        <svg class="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
                            ...
                            @if($isAmil)
                                <a href="{{ route('profil.show') }}"
                                   class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors
                                          {{ request()->routeIs('profil.*') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style="background: #f3f4f6;">
                                        <svg class="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                        ...
                        <div class="px-3 py-2" style="border-top: 1px solid #f0f7f0;">
                            <button type="button"
                                    ...
                                    class="w-full flex items-center space-x-3 px-3 py-2.5 rounded-xl text-[12.5px] font-medium text-red-600
...
<script>
    ...
    /**
     * Klik notif → fade out + badge berkurang + kirim ke server
     */
