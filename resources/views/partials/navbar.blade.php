...
@php
    ...
    if ($isAdminMasjid && $authUser?->masjid?->admin_foto) {
        ...
        // Muzakki: foto ada di model Muzakki
        ...
        // Fallback: foto langsung di tabel pengguna (superadmin dll)
...
<div id="logout-modal" class="fixed inset-0 z-[9999] flex items-center justify-center hidden" aria-modal="true"
    ...
    <div id="logout-card"
        class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden scale-90 opacity-0 transition-all duration-300"
        ...
        <div class="p-7">
            ...
            <h2 class="text-[17px] font-bold text-gray-900 text-center mb-1.5" style="letter-spacing: -0.02em;">Keluar
                dari Akun?</h2>
            <p class="text-sm text-gray-500 text-center leading-relaxed mb-7">Sesi Anda akan diakhiri. Pastikan semua
                pekerjaan sudah tersimpan sebelum keluar.</p>
            <div class="flex items-center space-x-3 px-4 py-3 rounded-xl mb-6"
                ...
                <span
                    class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold flex-shrink-0 {{ $badgeClass }}">{{ $roleLabel }}</span>
            </div>
            <div class="flex items-center space-x-3">
                <button onclick="closeLogoutModal()"
                    class="flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold text-gray-700 transition-all duration-200 hover:bg-gray-100 active:scale-95"
                    style="border: 1.5px solid #e5e7eb;">Batal</button>
                <form method="POST" action="{{ route('logout') }}" id="logout-form" class="flex-1">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center justify-center space-x-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white transition-all duration-200 active:scale-95"
        ...
        <div class="flex items-center justify-between" style="height: 60px;">
            ...
            <div class="flex items-center space-x-4">
                <div class="hidden lg:block">
                    <h1 class="text-[17px] font-bold text-gray-900 leading-tight" style="letter-spacing: -0.025em;">
            ...
            <div class="flex items-center space-x-1.5">
                ...
                @if ($isSuperadmin)
                    <a href="{{ route('superadmin.kontak.index') }}" title="Pesan Masuk dari Pengguna"
                        ...
                        @if ($kontakBelumDibaca > 0)
                            <span
                                class="absolute -top-0.5 -right-0.5 min-w-[17px] h-[17px] px-1 text-white text-[9px] font-bold rounded-full flex items-center justify-center leading-none"
                ...
                @if ($isAmil || $isMuzakki)
                    <div class="relative">
                        ...
                        <div id="notifications-dropdown"
                            class="absolute right-0 mt-2.5 w-[340px] bg-white rounded-2xl hidden overflow-hidden"
                            style="border: 1px solid #eef0f2">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-2.5">
                                        <div class="w-8 h-8 rounded-xl flex items-center justify-center"
                                            ...
                                            <svg class="w-4 h-4 text-[#2d6a2d]" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                                            </svg>
                            ...
                            <div id="notif-list" class="max-h-[380px] overflow-y-auto"
                                ...
                                @forelse($notifItems as $notif)
                                    <a href="{{ $notif['url'] }}" data-id="{{ $notif['id'] }}"
                                        ...
                                        class="notif-item flex items-start space-x-3 px-5 py-3.5 hover:bg-[#f9fbf9] transition-all duration-200 group"
                ...
                <div class="relative">
                    <button onclick="toggleUserMenu()" id="user-menu-btn"
                        ...
                        <div class="w-8 h-8 rounded-xl overflow-hidden flex-shrink-0 flex items-center justify-center"
                            style="background: #2d6a2d;">
                            @if ($avatarUrl)
                                <img src="{{ $avatarUrl }}" alt="{{ $authUser->username }}"
                                    class="w-full h-full object-cover">
                    ...
                    <div id="user-menu-dropdown"
                        class="absolute right-0 mt-2 w-56 bg-white rounded-xl hidden overflow-hidden"
                        style="border: 1px solid #ddeedd">
                            ...
                            <div class="flex items-center space-x-2.5">
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center gap-1.5 flex-wrap">
                                        ...
                                        <span
                                            class="inline-flex items-center px-1.5 py-0.5 rounded-md text-[9.5px] font-semibold {{ $badgeClass }}">{{ $roleLabel }}</span>
                                    </div>
                        ...
                        <div class="py-1">
                            @if ($isSuperadmin)
                                <a href="{{ route('superadmin.profil.show') }}"
                                    class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors {{ request()->routeIs('superadmin.profil.*') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                ...
                                <a href="{{ route('konfigurasi-global.show') }}"
                                    class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors {{ request()->routeIs('konfigurasi-global.*') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                                        style="background: #f3f4f6;">
                                        <svg class="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                ...
                                <a href="{{ route('superadmin.kontak.index') }}"
                                    class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors {{ request()->routeIs('superadmin.kontak.*') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                                        style="background: #f3f4f6;">
                                        <svg class="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                            ...
                            @if ($isAdminMasjid)
                                <a href="{{ route('admin-masjid.profil.show') }}"
                                    class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors {{ request()->routeIs('admin-masjid.profil.*') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                                        style="background: #f3f4f6;"><svg class="w-3.5 h-3.5 text-gray-500"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                        </svg></div>
                                ...
                                <a href="{{ route('konfigurasi-integrasi.show') }}"
                                    class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors {{ request()->routeIs('konfigurasi-integrasi.*') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                                        style="background: #f3f4f6;"><svg class="w-3.5 h-3.5 text-gray-500"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            @if ($isAmil)
                                <a href="{{ route('profil.show') }}"
                                    class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors {{ request()->routeIs('profil.*') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                                        style="background: #f3f4f6;"><svg class="w-3.5 h-3.5 text-gray-500"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            @if ($isMuzakki)
                                <a href="{{ route('muzakki.profil.show') }}"
                                    class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors
              {{ request()->routeIs('muzakki.profil.*') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                                        style="background: #f3f4f6;">
                                        <svg class="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                ...
                                <a href="{{ route('transaksi-daring-muzakki.create') }}"
                                    class="flex items-center space-x-3 px-4 py-2 text-[12.5px] text-gray-700 hover:bg-[#f5faf5] hover:text-[#1f5c1f] transition-colors
              {{ request()->routeIs('transaksi-daring-muzakki.create') ? 'bg-[#f0f7f0] text-[#1f5c1f] font-semibold' : '' }}">
                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                                        style="background: #f3f4f6;">
                                        <svg class="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
