    ...
    <div class="bg-white rounded-xl sm:rounded-2xl shadow-card border border-gray-100 overflow-hidden animate-slide-up">
        ...
        <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-base sm:text-lg font-semibold text-gray-900">Form Edit Bulletin</h2>
                    <p class="text-xs sm:text-sm text-gray-500 mt-1">Edit data bulletin "{{ Str::limit($bulletin->judul, 40) }}"</p>
                </div>
            </div>
        </div>
        ...
        @if($bulletin->isRejected() && $bulletin->rejection_reason)
            <div class="px-4 sm:px-6 py-3 bg-red-50 border-b border-red-200">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm font-semibold text-red-800">Bulletin Ditolak — Perbaiki dan Ajukan Ulang</p>
                        <p class="text-xs text-red-700 mt-0.5">
                            <span class="font-medium">Alasan penolakan:</span> {{ $bulletin->rejection_reason }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <form action="{{ route('admin-lembaga.bulletin.update', $bulletin->uuid) }}" method="POST"
            ...
            <div class="space-y-6">
                        ...
                        <div id="select-container">
                            <select id="kategori-select" name="kategori_bulletin_id"
                                ...
                                @foreach($kategoriList as $kat)
                                    <option value="{{ $kat->id }}"
                                        {{ old('kategori_bulletin_id', $bulletin->kategori_bulletin_id) == $kat->id ? 'selected' : '' }}>
                                        {{ $kat->nama_kategori }}
                                    </option>
                                @endforeach
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
                    <div>
                        ...
                        <input type="text" name="judul" id="judul"
                               value="{{ old('judul', $bulletin->judul) }}" required
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="lokasi" id="lokasi"
                                   value="{{ old('lokasi', $bulletin->lokasi) }}"
                                   placeholder="Contoh: Jakarta, Indonesia"
                                   class="block w-full pl-10 pr-3 py-2 text-sm border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div id="thumbnail-zone"
                        ...
                        <div id="thumbnail-placeholder">
                            ...
                            <p class="text-gray-500 text-sm font-medium mb-1">
                                {{ $bulletin->thumbnail ? 'Upload gambar baru untuk mengganti' : 'Klik untuk upload atau drag & drop' }}
                            </p>
                    ...
                    <div class="mt-3">
                        <input type="text" name="image_caption" id="image_caption"
                               value="{{ old('image_caption', $bulletin->image_caption) }}"
                ...
                <div>
                    <label for="published_at" class="block text-sm font-medium text-gray-700 mb-2">
                        Tanggal Publish <span class="text-gray-400 font-normal text-xs">(opsional)</span>
                    </label>
                    <input type="datetime-local" name="published_at" id="published_at"
                           value="{{ old('published_at', $bulletin->published_at?->format('Y-m-d\TH:i')) }}"
                    ...
                    <p class="mt-1 text-xs text-gray-400">Kosongkan untuk tetap menggunakan tanggal publish sebelumnya</p>
                    @error('published_at')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="konten" id="konten" class="hidden">{{ old('konten', $bulletin->konten) }}</textarea>
                    @error('konten')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 pt-6 border-t border-gray-200">
                    ...
                    <button type="button" id="btnDraft"
                            class="inline-flex items-center justify-center px-6 py-2.5 border border-gray-300 bg-white hover:bg-gray-50 text-gray-700 text-sm font-medium rounded-xl transition-colors">
...
{{-- Form hapus thumbnail (terpisah dari form utama) --}}
<form id="delete-thumbnail-form"
      action="{{ route('admin-lembaga.bulletin.delete-thumbnail', $bulletin->uuid) }}"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    try {
        ...
        const existing = {!! json_encode(old('konten', $bulletin->konten)) !!};
    ...
    function doSubmit(actionValue, btn, loadingLabel) {
        if (quill) {
        ...
        // Inject hidden action field
        let hidden = document.getElementById('_action_value');
        if (!hidden) {
            hidden = document.createElement('input');
            hidden.type = 'hidden';
            hidden.name = 'action';
            hidden.id   = '_action_value';
            form.appendChild(hidden);
        }
        hidden.value = actionValue;

        btn.disabled    = true;
        btn.innerHTML   = '<svg class="animate-spin h-4 w-4 mr-2 inline" fill="none" viewBox="0 0 24 24">'
    ...
    document.getElementById('btnDraft').addEventListener('click', function () {
        doSubmit('draft', this, 'Menyimpan...');
    });

    document.getElementById('btnSubmit').addEventListener('click', function () {
        doSubmit('submit', this, 'Mengirim...');
    });
    ...
    window.batalBuatKategori = function () {
        ...
        newKatInput.value = '';
        sel.value = '{{ $bulletin->kategori_bulletin_id }}';
    };

    // Auto-focus
    if (window.innerWidth >= 768) {
        setTimeout(() => { const el = document.getElementById('judul'); if (el) { el.focus(); el.select(); } }, 100);
    }
});
</script>
@endpush