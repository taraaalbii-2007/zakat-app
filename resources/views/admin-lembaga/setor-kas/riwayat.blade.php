                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                    ...
                    <div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
                        ...
                        <a href="{{ route('admin-lembaga.setor-kas.pending') }}"
                            ...
                            <span
                                class="hidden sm:inline-block sm:ml-2 group-hover:inline-block transition-all duration-300">Pending</span>
                        </a>
                        ...
                        <div id="search-container" class="transition-all duration-300"
                            ...
                            <form method="GET" action="{{ route('admin-lembaga.setor-kas.riwayat') }}" id="search-form"
                ...
                <form method="GET" action="{{ route('admin-lembaga.setor-kas.riwayat') }}" id="filter-form">
                    ...
                    @if (request()->hasAny(['status', 'dari', 'sampai']))
                        <div class="mt-3 flex justify-end">
                            <a href="{{ route('admin-lembaga.setor-kas.riwayat', request('q') ? ['q' => request('q')] : []) }}"
                            ...
                            @foreach ($setorans as $setor)
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs text-gray-400">{{ $setor->tanggal_setor->format('d M Y') }}
                                        </div>
                                    </td>
                                    {{-- Periode --}}
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900">{{ $setor->periode_dari->format('d M Y') }}
                                        </div>
                                        <div class="text-xs text-gray-500">s/d
                                            {{ $setor->periode_sampai->format('d M Y') }}</div>
                                    </td>
                                    {{-- Jumlah --}}
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-semibold text-gray-900">
                                            {{ $setor->jumlah_disetor_formatted }}</div>
                                        @if ($setor->jumlah_dihitung_fisik !== null)
                                    ...
                                    <td class="px-6 py-4">
                                        @if ($selisih !== null)
                                            @if ($selisih == 0)
                                                <span
                                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">Cocok</span>
                                            @elseif($selisih > 0)
                                                <span
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($setor->status === 'diterima')
                                            ...
                                            @if ($setor->diterima_at)
                                                <div class="text-xs text-gray-400 mt-1">
                                                    {{ \Carbon\Carbon::parse($setor->diterima_at)->format('d M Y, H:i') }}
                                                </div>
                                            @endif
                                        @elseif($setor->status === 'ditolak')
                                            ...
                                            @if ($setor->ditolak_at)
                                                <div class="text-xs text-gray-400 mt-1">
                                                    {{ \Carbon\Carbon::parse($setor->ditolak_at)->format('d M Y, H:i') }}
                                                </div>
                                            @endif
                                        @else
                                            <span
                    ...
                    @foreach ($setorans as $setor)
                        ...
                        <div class="p-4 hover:bg-gray-50 transition-colors">
                            <div class="flex items-start justify-between">
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex flex-wrap gap-1.5 mb-3">
                                        ...
                                        @if ($selisih !== null)
                                            ...
                                            @elseif($selisih > 0)
                                                <span
                                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700">+Rp
                                                    {{ number_format($selisih, 0, ',', '.') }}</span>
                                            @else
                                                <span
                                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700">-Rp
                                    ...
                                    <div class="space-y-1.5">
                                        <div class="flex items-center text-xs text-gray-600">
                                            <svg class="w-3.5 h-3.5 mr-1.5 flex-shrink-0 text-gray-400" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                            </svg>
                                            Periode: {{ $setor->periode_dari->format('d M Y') }} —
                                            {{ $setor->periode_sampai->format('d M Y') }}
                                        </div>
                                        <div class="flex items-center text-xs text-gray-600">
                                            <svg class="w-3.5 h-3.5 mr-1.5 flex-shrink-0 text-green-500" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                            <span
                                                class="font-semibold text-gray-800">{{ $setor->jumlah_disetor_formatted }}</span>
                                            @if ($setor->jumlah_dihitung_fisik !== null)
                                                <span class="ml-1 text-gray-400">(Fisik: Rp
                                                    {{ number_format($setor->jumlah_dihitung_fisik, 0, ',', '.') }})</span>
                                            @endif
                                        </div>
                                ...
                                <button type="button" data-uuid="{{ $setor->uuid }}" data-no="{{ $setor->no_setor }}"
                                    data-nama="{{ $setor->amil->nama_lengkap ?? ($setor->amil->pengguna->username ?? '-') }}"
                                    data-status="{{ $setor->status }}"
                                    data-alasan="{{ $setor->alasan_penolakan ?? '' }}"
    ...
    <div id="alasan-modal"
        ...
        <div class="p-6 border border-gray-200 w-full max-w-sm shadow-xl rounded-2xl bg-white">
            ...
            <div class="flex justify-center">
                <button type="button"
                    onclick="document.getElementById('alasan-modal').classList.add('hidden'); document.body.style.overflow='';"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            alasanBtn.addEventListener('click', function() {
                ...
                document.getElementById('modal-alasan-text').textContent = currentAlasan ||
